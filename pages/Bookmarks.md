# Bookmarks bar

[WhatsApp](https://web.whatsapp.com/)

[iCloud](https://www.icloud.com/)

[ArkamFahry](https://github.com/ArkamFahry)

[KnowledgeBase](https://arkamfahry.github.io/KnowledgeBase/#/)

[daily.dev | The Homepage Developers Deserve](https://app.daily.dev/?utm_source=extension&utm_medium=cta&utm_campaign=welcomeback&utm_id=uninstall)

[Bard](https://bard.google.com/)

[chat.openai.com](https://chat.openai.com/)

[Copilot](https://copilot.microsoft.com/)

[Bing AI - Search](https://www.bing.com/search?q=Bing+AI&showconv=1&FORM=hpcodx)

[Assistant - Poe](https://poe.com/)

[Claude](https://claude.ai/chats)

[Hashnode](https://hashnode.com/)

[Figma](https://www.figma.com/files/recent?fuid=1130475209500036894)

[Rive - The new standard for interactive graphics](https://rive.app/)

[Penpot - Design Freedom for Teams](https://design.penpot.app/#/auth/login)

[Piped](https://piped.video/feed)

[Fireship - Learn to Code Faster](https://fireship.io/)

[devmio - Software Know-How](https://devm.io/)

[Smashing Magazine — For Web Designers And Developers](https://www.smashingmagazine.com/)

[Learn Game Development | Godot Tutorials](https://godottutorials.com/)

[Open-source headless commerce with GraphQL, TypeScript and Node.js | Vendure docs](https://www.vendure.io/)

[Medusa - the open source Shopify alternative](https://medusajs.com/)

[Lunacy – Free Design Software for Win, Mac, Linux](https://icons8.com/lunacy)

[A Web Developer's Browser | Responsively App](https://responsively.app/)

[Feed - Piped](https://piped.video/feed)

[LeetCode - The World's Leading Online Programming Learning Platform](https://leetcode.com/)

[Where work happens | Slack](https://slack.com/)

[Firebase](https://firebase.google.com/)

[The Open Source Firebase Alternative | Supabase](https://supabase.com/)

[Nhost: The Open Source Firebase Alternative with GraphQL](https://nhost.io/)

[Back4App - Low-code backend to build modern apps](https://www.back4app.com/)

[MongoDB: The Developer Data Platform | MongoDB](https://www.mongodb.com/)

[Cockroach Labs, the company building CockroachDB](https://www.cockroachlabs.com/)

[The Distributed SQL Database for Cloud Native Applications | Yugabyte](https://www.yugabyte.com/)

[DataStax | NoSQL Database Built on Apache Cassandra | DataStax](https://www.datastax.com/)

[PlanetScale: The world’s most advanced database platform](https://planetscale.com/)

[Dgraph | GraphQL Cloud Platform – GraphQL . Javascript . Distributed Graph Engine | Deploy a Production Ready GraphQL Backend in Minutes](https://dgraph.io/)

[Fauna | The distributed serverless database](https://fauna.com/)

[Redis | The Real-time Data Platform](https://redis.com/)

[Auth0: Secure access for everyone. But not just anyone.](https://auth0.com/)

[Clerk | Authentication and User Management](https://clerk.dev/)

[Ory - Open Source Identity Solutions For Everyone](https://www.ory.sh/)

[SuperTokens, Open Source Authentication](https://supertokens.com/)

[Frontegg | User Management for modern B2B SaaS](https://frontegg.com/)

[Railway](https://railway.app/)

[Upstash: Serverless Data for Redis® and Kafka®](https://upstash.com/)

[Cloud Application Hosting for Developers | Render](https://render.com/)

[Develop. Preview. Ship. For the best frontend teams – Vercel](https://vercel.com/)

[Develop and deploy websites and apps in record time | Netlify](https://www.netlify.com/)

[Deploy app servers close to your users · Fly](https://fly.io/)

[8base | Platform and Services for Web and Mobile Apps](https://www.8base.com/)

[GraphQL backends made easy – Grafbase](https://grafbase.com/)

[Prisma | Next-generation ORM for Node.js & TypeScript](https://www.prisma.io/)

[The Easiest Way to Deploy Your App | Adaptable.io](https://adaptable.io/)

[Node.js App Development: Open Source Platform | Amplication](https://amplication.com/)

[PopSQL - Collaborative SQL editor for teams - Download our SQL tool for Mac, Windows, and Linux](https://popsql.com/)

[DrawSQL - 🔥 Database schema diagrams](https://drawsql.app/)

[dbdiagram.io - Database Relationship Diagrams Design Tool](https://dbdiagram.io/home)

[Luna Modeler - Database Design Tool | DATENSEN](https://www.datensen.com/)

[Appsmith | Open-source workbench for internal tools](https://www.appsmith.com/)

[Builder.io - Drag and drop Visual CMS](https://www.builder.io/)

[The One DevOps Platform | GitLab](https://about.gitlab.com/)

[Bitbucket | Git solution for teams using Jira](https://bitbucket.org/product/)

[JetBrains: Essential tools for software developers and teams](https://www.jetbrains.com/)

[Collaboration software for software, IT and business teams](https://www.atlassian.com/)

[Continuous Integration and Delivery - CircleCI](https://circleci.com/)

[Codemagic - CI/CD for Android, iOS, Flutter and React Native projects](https://codemagic.io/start/)

[Gitpod: Always ready to code.](https://www.gitpod.io/)

[Framer: Ship sites with style](https://www.framer.com/)

[Photopea | Online Photo Editor](https://www.photopea.com/)

[Chat Engine | Simple APIs & UI Kits](https://chatengine.io/)

[Pusher | Leader In Realtime Technologies](https://pusher.com/)

[Ably: the platform to power synchronized digital experiences in realtime | Ably Realtime](https://ably.com/)

[FlutterFlow - Build beautiful, modern apps incredibly fast!](https://flutterflow.io/)

[Stackhero: Managed cloud services provider (PaaS)](https://www.stackhero.io/en/)

[goormIDE - A Powerful Cloud IDE Service](https://ide.goorm.io/)

[StackBlitz | Instant Dev Environments | Click. Code. Done.](https://stackblitz.com/)

[Replit: the collaborative browser based IDE - Replit](https://replit.com/)

[Rapid - The Next Generation API Platform](https://rapidapi.com/)

[Postman API Platform | Sign Up for Free](https://www.postman.com/)

[The API Design Platform and API Client - Insomnia](https://insomnia.rest/)

[Mattermost | Secure Collaboration for Technical Teams](https://mattermost.com/)

[Revolt - Find Your Community](https://revolt.chat/)

[Jami](https://jami.net/)

[Rocket.Chat: Communications Platform You Can Fully Trust](https://www.rocket.chat/)

[Element | Secure collaboration and messaging](https://element.io/)

[Signal >> Home](https://signal.org/en/#signal)

[Nubo](https://nubo.app/)

[BagelDB | Content platform that gives you the power to build apps in record time](https://bageldb.com/)

[HarperDB | Simplicity Without Sacrifice](https://harperdb.io/)

[restdb.io - Simple online database backend with NoSQL - automatic REST API - dynamic web - MongoDB support - low code hooks - free development plan](https://restdb.io/)

[StepZen: GraphQL-as-a-Service: Build GraphQL faster, run better, scale seamlessly.](https://stepzen.com/)

[All-in-one project management tool - Height](https://height.app/)

[Obsidian](https://obsidian.md/)

[Logseq: A privacy-first, open-source knowledge base](https://logseq.com/)

[Unigraph - organize, relate, and discover everything in your life on your knowledge graph, all in one workspace](https://unigraph.dev/)

[Foam | A personal knowledge management and sharing system for VSCode](https://foambubble.github.io/foam/)

[RemNote](https://www.remnote.com/)

[AppFlowy.IO](https://appflowy.io/)

[Notion – One workspace. Every team.](https://www.notion.so/product)

[Basecamp: Project management software, online collaboration](https://basecamp.com/)

[Task List, Notes, Team Agenda, & Chat | Taskade](https://www.taskade.com/)

[ClickUp™ | One app to replace them all](https://clickup.com/)

[Craft - The Future of Documents](https://www.craft.do/)

[Cryptee | Encrypted Secure Photo Storage & Encrypted Documents Editor](https://crypt.ee/)

[Mem](https://mem.ai/)

[Best Note Taking App - Organize Your Notes with Evernote](https://evernote.com/)

[Neverinstall | Your browser is the new operating system](https://neverinstall.com/)

[Serverless database with powerful search for your Next.js app | Xata](https://xata.io/)

[Neon — Serverless, Fault-Tolerant, Branchable Postgres](https://neon.tech/)

[SurrealDB | The ultimate serverless cloud database](https://surrealdb.com/)

[EdgeDB | The post-SQL era has arrived](https://www.edgedb.com/)

[Home | Vaticle](https://vaticle.com/)

[SingleStoreDB is The Real-Time Distributed SQL Database, designed for data-intensive applications.](https://www.singlestore.com/)

[EventStoreDB - the state-transition database for data-driven businesses](https://www.eventstore.com/)

[QuestDB | Time series data, faster](https://questdb.io/)

[The Serverless API Developer Platform - WunderGraph](https://wundergraph.com/)

[Appwrite - Open-Source End-to-End Backend Server](https://appwrite.io/)

[Fission](https://fission.io/)

[Space Cloud: Instant Realtime APIs for Serverless Apps](https://space-cloud.io/)

[Parse Platform](https://parseplatform.org/)

[Para - Backend for busy developers.](https://paraio.com/signin)

[Convex](https://www.convex.dev/)

[API Documentation & Design Tools for Teams | Swagger](https://swagger.io/)

[Glitch: The friendly community where everyone builds the web](https://glitch.com/)

[Excalidraw](https://excalidraw.com/)

[tldraw](https://www.tldraw.com/)

[Witeboard](https://witeboard.com/)

[The Visual Collaboration Platform for Every Team | Miro](https://miro.com/)

[pREST | Instant RESTFul APIs for your data | Join data across databases, REST services to build powerful modern applications](https://prestd.com/)

[PostgREST Documentation — PostgREST 11.1.0 documentation](https://postgrest.org/en/stable/)

[Prostgles - Isomorphic TypeScript library for PostgreSQL](https://prostgles.com/)

[Graphile | Powerful, Extensible and Performant GraphQL APIs Rapidly](https://www.graphile.org/)

[Instant GraphQL APIs on your data | Built-in Authz & Caching | Hasura](https://hasura.io/)

[GraphJin - Build APIs in 5 minutes](https://graphjin.com/)

[Introduction - Join Monster](https://join-monster.readthedocs.io/en/latest/)

[RESTHeart - The most effective microservices framework](https://restheart.org/)

[Home | Stargate](https://stargate.io/)

[DreamFactory API management](https://www.dreamfactory.com/)

[Redpanda | The streaming data platform for developers](https://redpanda.com/)

[The Streaming Database | Materialize](https://materialize.com/)

[Meilisearch](https://www.meilisearch.com/)

[Typesense | Fast, typo-tolerant open source search engine](https://typesense.org/)

[AI Site Search & Discovery Solutions | Search](https://www.search.io/)

[Site Search & Discovery powered by AI | Algolia](https://www.algolia.com/)

[Debezium](https://debezium.io/)

[GraphQL Tools – The Guild](https://the-guild.dev/)

[Etebase - Your end-to-end encrypted backend](https://www.etebase.com/)

[No vendor-lock-in backend as a service | StaticBackend](https://staticbackend.com/)

[PocketBase - Open Source backend in 1 file](https://pocketbase.io/)

[Lenra - Optimize your app creation](https://www.lenra.io/)

[Heroic Labs | GameTech for Studios and Developers](https://heroiclabs.com/)

[Authorizer | Your data your control](https://authorizer.dev/)

[Auth. Built for Devs, by Devs - FusionAuth](https://fusionauth.io/)

[Gluu Identity and Access Management](https://gluu.org/)

[ZITADEL • The best of Auth0 and Keycloak combined. Built for the serverless era](https://zitadel.com/)

[Authelia - The Single Sign-On Multi-Factor portal for web apps](https://www.authelia.com/)

[Hanko - Open source authentication beyond passwords](https://www.hanko.io/)

[Cyclic.sh - Fullstack Javascript Apps - Deploy and Host in Seconds](https://www.cyclic.sh/)

[Deno Deploy](https://deno.com/deploy)

[twitter.com](https://twitter.com/)

[Create and build your online business with PrestaShop](https://www.prestashop.com/en)

[thirty bees open source ecommerce platform - e-commerce that works](https://thirtybees.com/)

[Live Projects | Litekart](https://litekart.in/)

[Replicache](https://replicache.dev/)

[soketi](https://soketi.app/)

[Drifting in Space](https://driftingin.space/)

[Begin](https://begin.com/)

[SlashApi - Build REST APIs in minutes](https://slashapi.com/)

[NocoDB | Turns your SQL database into a Nocode platform. Free & Open Source.](https://www.nocodb.com/)

[XTDB](https://xtdb.com/)

[Saleor – A headless, GraphQL-first, open-source e-commerce platform](https://saleor.io/)

[ObjectBox](https://objectbox.io/)

[The world’s fastest framework for building websites | Hugo](https://gohugo.io/)

[Zola](https://www.getzola.org/)

[RedwoodJS: The App Framework for Startups | RedwoodJS.com](https://redwoodjs.com/)

[Create T3 App](https://create.t3.gg/)

[Laravel - The PHP Framework For Web Artisans](https://laravel.com/)

[Phoenix Framework](https://www.phoenixframework.org/)

[Blitz.js - The Missing Fullstack Toolkit for Next.js](https://blitzjs.com/)

[Meteor Software: A Platform to Build, Host, Deploy and Scale Full-Stack Javascript Applications](https://www.meteor.com/)

[AdonisJS - A fully featured web framework for Node.js](https://adonisjs.com/)

[NestJS - A progressive Node.js framework](https://nestjs.com/)

[LoopBack](https://loopback.io/)

[Remult](https://remult.dev/)

[FactorJS Static Site Framework by Kaption Co.](https://www.factorjs.org/)

[Nullstack - Feature-Driven Full Stack JavaScript Components](https://nullstack.app/)

[Layr – Dramatically simplify full‑stack development](https://layrjs.com/)

[Apollo GraphQL | Supergraph: unify APIs, microservices, & databases in a composable graph](https://www.apollographql.com/)

[hapi.dev - The simple, secure framework developers trust](https://hapi.dev/)

[tRPC - Move Fast and Break Nothing. End-to-end typesafe APIs made easy. | tRPC](https://trpc.io/)

[Vite | Next Generation Frontend Tooling](https://vitejs.dev/)

[SvelteKit • Web development, streamlined](https://kit.svelte.dev/)

[Astro | Build faster websites](https://astro.build/)

[Framework reimagined for the edge! - Qwik](https://qwik.builder.io/)

[Next.js by Vercel - The React Framework](https://nextjs.org/)

[Nuxt - The Intuitive Vue Framework](https://nuxtjs.org/)

[Remix - Build Better Websites](https://remix.run/)

[SolidJS · Reactive Javascript Library](https://www.solidjs.com/)

[Analog | Analog](https://analogjs.org/)

[Build smaller, faster, and more secure desktop applications with a web frontend | Tauri Apps](https://tauri.app/)

[Flutter - Build apps for any screen](https://flutter.dev/)

[Tailwind CSS - Rapidly build modern websites without ever leaving your HTML.](https://tailwindcss.com/)

[Fiber](https://gofiber.io/)

[gqlgen](https://gqlgen.com/)

[Gin Web Framework](https://gin-gonic.com/)

[livebud/bud: The Full-Stack Web Framework for Go](https://github.com/livebud/bud)

[Home | Salvo](https://salvo.rs/)

[Actix](https://actix.rs/)

[anime.js • JavaScript animation engine](https://animejs.com/)

[Theatre.js - animation toolbox for the web](https://www.theatrejs.com/)

[Three.js – JavaScript 3D Library](https://threejs.org/)

[ueli - A keystroke launcher for Windows and macOS](https://ueli.app/#/)

[Spacedrive — A file manager from the future.](https://www.spacedrive.com/)

[Fig](https://fig.io/)

[Warp: The terminal for the 21st century](https://www.warp.dev/)

[Budibase | Build internal tools in minutes, the easy way](https://budibase.com/)

[Free for developers](https://free-for.dev/#/)

[🪴 Quartz 3.](https://quartz.jzhao.xyz/)

[Agones](https://agones.dev/site/)

[Multiplayer server | Colyseus: Simple & Fast Multiplayer Game Creation](https://www.colyseus.io/)

[DerbyJS](https://derbyjs.com/)

[Feathers | A framework for real-time applications and REST APIs](https://feathersjs.com/)

[Socket.IO](https://socket.io/)

[deepstream.io](https://deepstream.io/)

[PouchDB, the JavaScript Database that Syncs!](https://pouchdb.com/)

[Build amazing real‑time collaborative products - Liveblocks](https://liveblocks.io/)

[RxDB - A client side, offline-first, reactive database for JavaScript Applications](https://rxdb.info/)

[Actionhero](https://www.actionherojs.com/)

[SocketCluster · Highly scalable pub/sub and RPC toolkit optimized for async/await](https://socketcluster.io/)

[dagger.io](https://dagger.io/)

[Dapr - Distributed Application Runtime](https://dapr.io/)

[DEV Community 👩‍💻👨‍💻](https://dev.to/)

[Indie Hackers: Work Together to Build Profitable Online Businesses](https://www.indiehackers.com/)

[Aiven - Data infrastructure made simple](https://aiven.io/)

[Memgraph - Open Source Graph Database](https://memgraph.com/)

[Tana – The Everything OS](https://tana.inc/)

[Cube — Headless BI for Building Data Applications](https://cube.dev/)

[End-to-end Machine Learning for Everyone - PostgresML](https://postgresml.org/)

[Baserow: Open source no-code database and Airtable alternative](https://baserow.io/)

[Database CI/CD | Database DevOps | Bytebase](https://www.bytebase.com/)

[Stellate - The GraphQL company](https://stellate.co/)

[Cosmic | The #1 Headless, API-first, Content Management Platform](https://www.cosmicjs.com/)

[Weaviate Vector Search Engine](https://weaviate.io/)

[Snyk | Developer security | Develop fast. Stay secure.](https://snyk.io/)

[LunaSec - Open Source Data Security Platform](https://www.lunasec.io/)

[Cloudflare - The Web Performance & Security Company | Cloudflare](https://www.cloudflare.com/)

[Genji](https://genji.dev/)

[authzed: the managed permissions database for everyone](https://authzed.com/)

[Real-Time In-App Chat and Communication Platform | PubNub](https://www.pubnub.com/)

[Zeplo - Magical HTTP request queue](https://www.zeplo.io/)

[Jamsocket: Deploy webapps that give every user a backend](https://www.jamsocket.com/)

[Plane Docs](https://plane.dev/)

[Airbyte | Open-Source Data Integration Platform | ELT tool](https://airbyte.com/)

[dbt - Transform data in your warehouse](https://www.getdbt.com/)

[Amundsen, the leading open source data catalog](https://www.amundsen.io/)

[Dagster | Cloud-native orchestration of data pipelines](https://dagster.io/)

[PostHog - The open source Product OS](https://posthog.com/)

[Matomo Analytics - The Google analytics Alternative that protects your data - variation - Analytics Platform - Matomo](https://matomo.org/matomo-analytics-the-google-analytics-alternative-that-protects-your-data-variation/)

[Customer Data Platform for Developers | RudderStack.com](https://www.rudderstack.com/)

[Plausible Analytics | Simple, privacy-friendly Google Analytics alternative](https://plausible.io/)

[Cal.com | Open Scheduling Infrastructure](https://cal.com/)

[Open Source Data Pipeline Management | Meltano](https://meltano.com/)

[Prefect - The New Standard in Dataflow Automation - Prefect](https://www.prefect.io/)

[Great Expectations Home Page • Great Expectations](https://greatexpectations.io/)

[Homepage | Temporal](https://temporal.io/)

[Bitwarden Open Source Password Manager | Bitwarden](https://bitwarden.com/)

[Ghost: The Creator Economy Platform](https://ghost.org/)

[Metabase | Business Intelligence, Dashboards, and Data Visualization](https://www.metabase.com/)

[Redash helps you make sense of your data](https://redash.io/)

[Lightdash - BI for teams that move fast](https://www.lightdash.com/)

[Chartbrew - Your most important metrics in one place](https://chartbrew.com/)

[DoltHub Home | DoltHub](https://www.dolthub.com/)

[ScyllaDB | Monstrously Fast + Scalable NoSQL](https://www.scylladb.com/)

[Realm Home | Realm.io](https://realm.io/)

[rqlite/rqlite: The lightweight, distributed relational database built on SQLite](https://github.com/rqlite/rqlite)

[What is RisingWave? | RisingWave](https://www.risingwave.dev/docs/current/intro/)

[Qdrant - Vector Search Engine](https://qdrant.tech/)

[Skytable: A free and open-source NoSQL database to build powerful apps](https://skytable.io/)

[Databend - The modern cloud data warehouse that empowers your object storage for real-time analytics. | Databend](https://databend.rs/)

[Deploy and scale realtime apps • Hop](https://hop.io/)

[Push Notification Software to Improve Customer Engagement - OneSignal](https://onesignal.com/)

[Communication APIs for SMS, Voice, Video & Authentication | Twilio](https://www.twilio.com/)

[Email Delivery, API, Marketing Service | SendGrid](https://sendgrid.com/)

[Stripe | Payment Processing Platform for the Internet](https://stripe.com/)

[Dark](https://darklang.com/)

[IoT Platform & Backend Open Source - Kuzzle](https://kuzzle.io/)

[Time-series data simplified | Timescale](https://www.timescale.com/)

[InfluxDB Times Series Data Platform | InfluxData | InfluxData](https://www.influxdata.com/)

[Canonical Dqlite](https://dqlite.io/)

[Litestream - Streaming SQLite Replication](https://litestream.io/)

[Home - PocketHost](https://pockethost.io/)

[Open Source Alternatives to Proprietary Software](https://www.opensourcealternative.to/)

[AFFiNE - All In One Workos](https://affine.pro/)

[HStreamDB: Modern Streaming Database for Data in Motion](https://hstream.io/)

[Tigris Data](https://www.tigrisdata.com/)

[Aerobase | Open Source Identity & Access Management](https://aerobase.io/)

[Welcome | authentik](https://goauthentik.io/)

[Cerbos | Open source, decoupled access control for your software](https://cerbos.dev/)

[Trino | Distributed SQL query engine for big data](https://trino.io/)

[TerminusDB an open-source in-memory document graph database](https://terminusdb.com/)

[Dragonfly](https://dragonflydb.io/)

[Rowy - Low-code backend on Google Cloud and Firebase](https://www.rowy.io/)

[Sandstorm](https://sandstorm.io/)

[CrowdSec - The open-source & collaborative IPS](https://www.crowdsec.net/)

[Machine Learning In Your Database Using SQL | mindsdb](https://mindsdb.com/)

[Manticore Search](https://manticoresearch.com/)

[Grist | The Evolution of Spreadsheets](https://www.getgrist.com/)

[Welcome to Redwood! - Redwood](https://docs.redwood.dev/)

[Tinode instant messenger](https://tinode.co/)

[LocalStack - A fully functional local cloud stack](https://localstack.cloud/)

[Oso: Authorization as a Service](https://www.osohq.com/)

[Casdoor · An Open Source UI-first Identity Access Management (IAM) / Single-Sign-On (SSO) platform supporting OAuth 2.0, OIDC, SAML and CAS | Casdoor · An Open Source UI-first Identity Access Management (IAM) / Single-Sign-On (SSO) platform supporting OAuth 2.0, OIDC, SAML and CAS](https://casdoor.org/)

[Casbin · An authorization library that supports access control models like ACL, RBAC, ABAC for Golang, Java, C/C++, Node.js, Javascript, PHP, Laravel, Python, .NET (C#), Delphi, Rust, Ruby, Swift (Objective-C), Lua (OpenResty), Dart (Flutter) and Elixir | Casbin](https://casbin.org/)

[Identity & Access Management Platform | Curity Identity Server](https://curity.io/)

[KVdb - Key-Value Store as a Service API for Serverless Apps](https://kvdb.io/)

[Dkron - Cloud native job scheduling system | Dkron](https://dkron.io/)

[Insight-led web analytics | Volument](https://volument.com/)

[RonDB - World's fastest Key-Value Store](https://www.rondb.com/)

[Flowchart Fun — Fast, Free Online Flowchart Maker](https://flowchart.fun/)

[The Wails Project | Wails](https://wails.io/)

[HackerNoon - read, write and learn about any technology](https://hackernoon.com/)

[Asgardeo](https://wso2.com/asgardeo/)

[PingCAP, the company building TiDB | SQL at Scale](https://www.pingcap.com/)

[The Open Source Twilio Alternative | Fonoster](https://fonoster.com/)

[Jitsu : Open Source Data Integration Platform](https://jitsu.com/)

[Fast Open-Source OLAP DBMS - ClickHouse](https://clickhouse.com/)

[Trusted Open Source PostgreSQL & Commercial Support for the Enterprise | Crunchy Data](https://www.crunchydata.com/)

[Marketing, Automation & Email Platform | Mailchimp](https://mailchimp.com/)

[Welcome - Postal Documentation](https://docs.postalserver.io/)

[Sketch · Design, collaborate, prototype and handoff](https://www.sketch.com/)

[Dioxus | An elegant GUI library for Rust](https://dioxuslabs.com/)

[Manage projects faster & collaborate better | Hive](https://hive.com/)

[DbGate | Open Source (no)SQL Database Client](https://dbgate.org/)

[Antares SQL | Free and Open Source Client](https://antares-sql.app/)

[The SQL Editor and Database Manager Of Your Dreams | Beekeeper Studio](https://www.beekeeperstudio.io/)

[Quadratic - The Data Science Spreadsheet](https://www.quadratichq.com/)

[Tiny Function](https://tinyfunction.com/)

[Pathfix | Integration Platform for OAuth and API Automation](https://pathfix.com/)

[Macrometa - Build Real-Time Applications and APIs](https://www.macrometa.com/)

[Firecracker](https://firecracker-microvm.github.io/)

[Home - FerretDB](https://www.ferretdb.io/)

[AWESOME-APP - Rust Templates for Awesome Applications (Tauri, SurrealDB, Kubernetes)](https://awesomeapp.org/)

[YDB — an open-source Distributed SQL Database](https://ydb.tech/)

[Lucky - Web framework for Crystal](https://luckyframework.org/)

[Ruby on Rails — A web-app framework that includes everything needed to create database-backed web applications according to the Model-View-Controller (MVC) pattern.](https://rubyonrails.org/)

[Lit](https://lit.dev/)

[Stencil](https://stenciljs.com/)

[Alpine.js](https://alpinejs.dev/)

[Inertia.js - The Modern Monolith](https://inertiajs.com/)

[Livewire | Laravel Livewire](https://laravel-livewire.com/)

[HTML Over The Wire | Hotwire](https://hotwired.dev/)

[Wireflow - Free Wire / User Flow Tool](https://wireflow.co/)

[Polypane, The browser for ambitious developers](https://polypane.app/)

[Rocket - Simple, Fast, Type-Safe Web Framework for Rust](https://rocket.rs/)

[okdb - Real-time API and Instant GraphQL Backend](https://okdb.io/)

[Never Build Permissions Again | Permit.io](https://www.permit.io/)

[PolyScale.ai](https://www.polyscale.ai/)

[daisyUI — Tailwind CSS Components](https://daisyui.com/)

[Quill - Your powerful rich text editor](https://quilljs.com/)

[Trix: A rich text editor for everyday writing](https://trix-editor.org/)

[Editor.js](https://editorjs.io/)

[GQty](https://gqty.dev/)

[Neo4j Graph Data Platform | Graph Database Management System](https://neo4j.com/)

[Plasmic - The headless page builder for your codebase](https://www.plasmic.app/)

[Zod | Documentation](https://zod.dev/)

[Snaplet](https://www.snaplet.dev/)

[Linear – A better way to build products](https://linear.app/)

[Product Hunt – The best new products in tech.](https://www.producthunt.com/)

[Sonuum — The audio editor for everyone](https://sonuum.com/)

[Bangle.io](https://bangle.io/)

[Stackbit/Site Builder: better websites and apps, faster.](https://www.stackbit.com/)

[esbuild - An extremely fast JavaScript bundler](https://esbuild.github.io/)

[Turbo](https://turbo.build/)

[Rust-based platform for the Web – SWC](https://swc.rs/)

[Nx: Smart, Fast and Extensible Build System](https://nx.dev/)

[Documentation | Lerna](https://lerna.js.org/)

[Lapce](https://lapce.dev/)

[Helix](https://helix-editor.com/)

[Rows — The new way to spreadsheet](https://rows.com/)

[Learn Laravel | Laracasts](https://laracasts.com/)

[Expert led courses for front-end web developers. | egghead.io](https://egghead.io/)

[KeystoneJS: The superpowered Node.js Headless CMS for developers - Keystone 6](https://keystonejs.com/)

[marmelab/awesome-rest: A collaborative list of great resources about RESTful API architecture, development, test, and performance](https://github.com/marmelab/awesome-rest)

[Altogic | Backend as a service platform](https://www.altogic.com/)

[subZero — GraphQL & REST API for your database](https://subzero.cloud/)

[Devii - An instant API for your app](https://www.devii.io/)

[Find Open Source By Searching, Browsing and Combining 177,148 Topics Across 59 Categories And 6,127,009 Projects | Awesome Open Source](https://awesomeopensource.com/)

[SeaQL - Building data intensive applications in Rust](https://www.sea-ql.org/)

[Diesel is a Safe, Extensible ORM and Query Builder for Rust](https://diesel.rs/)

[GORM - The fantastic ORM library for Golang, aims to be developer friendly.](https://gorm.io/)

[ent](https://entgo.io/)

[The platform to bring your best ideas to life | Contentful](https://www.contentful.com/)

[The Next Generation GraphQL Content Platform | Hygraph](https://hygraph.com/)

[Strapi - Open source Node.js Headless CMS 🚀](https://strapi.io/)

[Directus: The Modern Data Stack, Democratized](https://directus.io/)

[Tinybird · The Way to build Real-time Data Products](https://www.tinybird.co/)

[Webhooks as a Service – Secure and Enterprise Ready · Svix](https://www.svix.com/)

[Sails.js | Realtime MVC Framework for Node.js](https://sailsjs.com/)

[MailDev - Create better emails during development](https://maildev.github.io/maildev/)

[mdSilo](https://mdsilo.com/)

[Padloc - Open Source, End-to-end Encrypted Password Manager](https://padloc.app/)

[Free utilities for developers | Echoo](https://web.echoo.app/)

[Conduit](https://getconduit.dev/)

[The Composable Content Cloud - Sanity.io](https://www.sanity.io/)

[Tooljet | Open-source low-code platform to build internal tools](https://www.tooljet.com/)

[Sheety - Turn your Google Sheet into an API](https://sheety.co/)

[Platformatic](https://platformatic.dev/)

[Hydra / The Open Source Data Warehouse](https://hydra.so/)

[Prisma Schema Builder](https://www.prismabuilder.io/)

[ReactiveSearch.io - LowCode Search with Elasticsearch](https://www.reactivesearch.io/)

[StackBricks](https://stackbricks.io/)

[Hippobase](https://hippobase.io/)

[Spawn · Instant database copies for development and CI](https://spawn.cc/)

[clientdb - OS in-memory database for real-time web apps](https://clientdb.dev/)

[The Biggest Video Game Database on RAWG - Video Game Discovery Service](https://rawg.io/)

[Dusk-Labs/dim: Dim, a media manager fueled by dark forces.](https://github.com/Dusk-Labs/dim)

[The Unison language](https://www.unison-lang.org/)

[Convert JSON to Swift, C#, TypeScript, Objective-C, Go, Java, C++ and more • quicktype](https://quicktype.io/)

[ZenML Home 🙏](https://zenml.io/home)

[SeMI Technologies](https://www.semi.technology/)

[Jina AI: MLOps for Multimodal AI, Neural Search, Generative AI, Creative AI](https://jina.ai/)

[DigitalOcean | The Cloud for Builders](https://www.digitalocean.com/)

[SSD VPS Servers, Cloud Servers and Cloud Hosting - Vultr.com](https://www.vultr.com/)

[Youki - Youki User and Developer Documentation](https://containers.github.io/youki/)

[Home | OpenFaaS - Serverless Functions Made Simple](https://www.openfaas.com/)

[Qovery - Create Production-like Environments at the Speed of Light](https://www.qovery.com/)

[Porter | Home](https://porter.run/)

[Deploy docker containers to the edge - Global container clusters close to your users | appfleet](https://appfleet.com/)

[Kyma - An easy way to extend enterprise applications on Kubernetes.](https://kyma-project.io/)

[Kong API Gateway and Service Connectivity Platform](https://konghq.com/)

[Traefik Labs: Makes Networking Boring](https://traefik.io/)

[TigerBeetle - A Million Transactions Per Second](https://tigerbeetle.com/)

[Marten | .NET Transactional Document DB and Event Store on PostgreSQL](https://martendb.io/)

[Unum | Scaling Intelligence](https://unum.cloud/)

[GitHub - TanmoySG/wunderDB: A micro JSON-based Data Store inspired by MongoDB.](https://github.com/TanmoySG/wunderDB)

[Lowdefy - Build internal tools with YAML | Lowdefy - Build internal tools with YAML](https://lowdefy.com/)

[Open-Source Serverless CMS for Enterprises - Headless CMS & Page Builder | Webiny](https://www.webiny.com/)

[GitHub - openblocks-dev/openblocks: 🔥 🔥 🔥 The Open Source Retool Alternative](https://github.com/openblocks-dev/openblocks)

[Frappe Framework: Open Source Low Code Framework](https://frappeframework.com/)

[NocoBase —— Open source and free no-code & low-code development platform](https://www.nocobase.com/)

[GrapesJS](https://grapesjs.com/)

[Codux | Visual IDE for React](https://www.codux.com/)

[No Code Kubernetes | Drag & Drop Kubernetes Deployment Tool | harpoon](https://www.harpoon.io/)

[Home - DGS Framework](https://netflix.github.io/dgs/)

[Lexical](https://lexical.dev/)

[🧭 Quickstart - flutter\_rust\_bridge](https://cjycode.com/flutter_rust_bridge/quickstart.html)

[Fusio - Open source API management platform](https://www.fusio-project.org/)

[Search more with less | Quickwit](https://quickwit.io/)

[idaas.nl](https://www.idaas.nl/)

[Logto: An open-source solution that creates a del…](https://logto.io/)

[Bitloops - Build enterprise-grade backends 10x fa…](https://bitloops.com/)

[Coolify](https://coolify.io/)

[Home - Clever Cloud](https://www.clever-cloud.com/)

[Multi-Cloud Infrastructure made easy | Multy](https://multy.dev/)

[Nebula · Docker orchestrator for IoT & distribute…](https://nebula-orchestrator.github.io/)

[React Auth Kit](https://authkit.arkadip.dev/)

[Introduction · GitBook | Juniper](https://graphql-rust.github.io/)

[Workona | The work organizer for the browser](https://workona.com/)

[shuttle](https://www.shuttle.rs/)

[Sycamore](https://sycamore-rs.netlify.app/)

[Yew](https://yew.rs/)

[lettre](https://lettre.rs/)

[Unlayer | Free email editor to build responsive email templates](https://unlayer.com/)

[GitBook - Where technical teams document.](https://www.gitbook.com/)

[Build optimized websites quickly, focus on your content | Docusaurus](https://docusaurus.io/)

[Formidable](https://formidable.com/)

[Atomic Data](https://atomicdata.dev/)

[Fluvio - The programmable data streaming platform](https://www.fluvio.io/)

[rust-unofficial/awesome-rust: A curated list of Rust code and resources.](https://github.com/rust-unofficial/awesome-rust)

[avelino/awesome-go: A curated list of awesome Go frameworks, libraries and software](https://github.com/avelino/awesome-go)

[Bulma: Free, open source, and modern CSS framework based on Flexbox](https://bulma.io/)

[Home | Windi CSS](https://windicss.org/)

[Home | Slidev](https://sli.dev/)

[VueUse | VueUse](https://vueuse.org/)

[Vitest | A blazing fast unit test framework powered by Vite](https://vitest.dev/)

[JavaScript End to End Testing Framework | cypress.io testing tools](https://www.cypress.io/)

[Bootstrap · The most popular HTML, CSS, and JS library in the world.](https://getbootstrap.com/)

[UIkit](https://getuikit.com/)

[Chakra UI - A simple, modular and accessible component library that gives you the building blocks you need to build your React applications. - Chakra UI](https://chakra-ui.com/)

[AppShell demo | Mantine](https://mantine.dev/)

[TDengine | Time Series Database Open Source TSDB](https://tdengine.com/)

[Prometheus - Monitoring system & time series database](https://prometheus.io/)

[Enterprise ML/NLP Products and Solutions for Semantic Search and Question Answering | deepset](https://www.deepset.ai/)

[Azimutt · Database explorer and analyzer](https://azimutt.app/)

[Authgear - Secure and Simple User Management](https://www.authgear.com/)

[Grafana: The open observability platform | Grafana Labs](https://grafana.com/)

[Tsuru](https://tsuru.io/)

[CapRover · Free and Open Source PaaS!](https://caprover.com/)

[Dokku - The smallest PaaS implementation you've ever seen](https://dokku.com/)

[Gitea](https://gitea.io/en-us/)

[Gogs: A painless self-hosted Git service](https://gogs.io/)

[Projects - OneDev](https://code.onedev.io/)

[Codeberg.org](https://codeberg.org/)

[n8n.io - a powerful workflow automation tool](https://n8n.io/)

[The Community-led Growth Platform for Developer Tools | crowd.dev](https://www.crowd.dev/)

[Mercure.rocks: Real-time APIs Made Easy](https://mercure.rocks/)

[JSON-to-Go: Convert JSON to Go instantly](https://mholt.github.io/json-to-go/)

[Avatar generator playground - Boring Avatars](https://boringavatars.com/)

[DiceBear Avatars](https://avatars.dicebear.com/)

[The Best Unlimited Online Backup and Cloud Storage Services | Backblaze](https://www.backblaze.com/)

[Flowbite - Build websites even faster with components on top of Tailwind CSS](https://flowbite.com/)

[Saltcorn - open-source no-code](https://saltcorn.com/)

[Datasette: An open source multi-tool for exploring and publishing data](https://datasette.io/)

[Atlas | Open-source database schema management tool](https://atlasgo.io/)

[zeeSQL, SQL and search by value for redis. Fast, simple and reliable.](https://zeesql.com/)

[Because Milliseconds Matter - Volt Active Data](https://www.voltactivedata.com/)

[Hazelcast | The Real-Time Data Platform](https://hazelcast.com/)

[Tamagui — React Native + Web UI kit](https://tamagui.dev/)

[HashiCorp: Infrastructure enables innovation](https://www.hashicorp.com/)

[Cost-effective Observability Platform | Uptrace](https://uptrace.dev/)

[VictoriaMetrics: Simple & Reliable Monitoring for Everyone](https://victoriametrics.com/)

[Echo - High performance, minimalist Go web framework](https://echo.labstack.com/)

[Bun — fast all-in-one JavaScript runtime](https://bun.sh/)

[Zanzibar: A Global Authorization System - Presented by Auth0](https://zanzibar.academy/)

[Fine Grained Authorization | OpenFGA](https://openfga.dev/)

[Permify | Open-source authorization service and policy engine](https://www.permify.co/)

[Topaz | Topaz](https://www.topaz.sh/)

[Kubernetes Monitoring, Application Debug Platform | Pixie](https://px.dev/)

[Open Policy Agent + Cloud-Native Authorization | Styra](https://www.styra.com/)

[Slashbase - The in-browser collaborative database IDE](https://slashbase.com/)

[Pothos GraphQL Overview and documentation](https://pothos-graphql.dev/)

[GitHub - IvanGoncharov/graphql-voyager: 🛰️ Represent any GraphQL API as an interactive graph | GraphQL Voyager](https://github.com/IvanGoncharov/graphql-voyager#graphql-voyager)

[GraphQL Editor: Online Code Editor & IDE for GraphQL development](https://graphqleditor.com/)

[Web3 Wallet Onboarding & Auth SDKs | Magic](https://magic.link/)

[Free and Open Search: The Creators of Elasticsearch, ELK & Kibana | Elastic](https://www.elastic.co/)

[KeyDB - The Faster Redis Alternative](https://docs.keydb.dev/)

[Vitess | Scalable. Reliable. MySQL-compatible. Cl…](https://vitess.io/)

[Citus Data | Distributed Postgres. At any scale.](https://www.citusdata.com/)

[Docker: Accelerated, Containerized Application Development](https://www.docker.com/)

[Kubernetes](https://kubernetes.io/)

[etcd](https://etcd.io/)

[Home - Knative](https://knative.dev/docs/)

[Red Hat - We make open source technologies for the enterprise](https://www.redhat.com/en)

[Serverless: Develop & Monitor Apps On AWS Lambda](https://www.serverless.com/)

[Cloud Computing Services  |  Google Cloud](https://cloud.google.com/)

[Cloud Computing Services | Microsoft Azure](https://azure.microsoft.com/en-us/)

[Cloud Computing Services - Amazon Web Services (AWS)](https://aws.amazon.com/)

[Your AWS, but 97% easier | StackTape](https://stacktape.com/)

[ComputeStacks](https://www.computestacks.com/)

[Emitter: Scalable Real-Time Communication Across Devices](https://emitter.io/)

[Kubero Kubernetes CI/CD PaaS](https://www.kubero.dev/)

[Nitric Cloud-Native Framework | Get Infrastructure from Code](https://nitric.io/)

[Suborbital | Extension Engine](https://suborbital.dev/)

[Universal Micro Services | M3O](https://m3o.com/)

[Nuclio: Serverless Platform for Automated Data Science](https://nuclio.io/)

[Backendless | Visual App Development Platform UI Builder and Backend](https://backendless.com/)

[Entity Signal - Real Time Entity Framework (EF) and ASP.NET core](https://entitysignal.com/)

[Expo](https://expo.dev/)

[Innovation Platform For Blockchain Development | Ignite CLI](https://ignite.com/)

[Ionic Framework - The Cross-Platform App Development Leader](https://ionicframework.com/)

[NativeScript](https://nativescript.org/)

[Envoy Proxy - Home](https://www.envoyproxy.io/)

[Caddy - The Ultimate Server with Automatic HTTPS](https://caddyserver.com/)

[Duende Software](https://duendesoftware.com/)

[Zalter - Passwordless authentication system](https://www.zalter.com/)

[Webhook Infrastructure and Tooling - Hookdeck](https://hookdeck.com/)

[NATS.io – Cloud Native, Open Source, High-performance Messaging](https://nats.io/)

[Uber Open Source](https://uber.github.io/#/)

[Radicle: Sovereign code infrastructure](https://radicle.xyz/)

[Open-Source UI elements - made with CSS and HTML | uiverse](https://uiverse.io/)

[AI Query | Use AI To Create SQL Queries In Seconds](https://www.aiquery.co/)

[AirOps | Data unlocked. ‍10x faster with AI.](https://www.airops.com/)

[Equals | The fastest way for startups to do any analysis](https://equals.app/)

[AskEdith - The Natural Language Interface for Cloud Data](https://www.askedith.ai/)

[CodeImage - A tool to manage and beautify your code screenshots](https://codeimage.dev/)

[AutoCloud: Build, Visualize, Track, and Query Cloud Infrastructure](https://www.autocloud.io/?ref=producthunt)

[Nimbus - Remote dev infra for teams](https://www.usenimbus.com/)

[Instant GraphQL backend to build fast and scale large | Agoston UI](https://agoston.io/)

[Rook](https://rook.io/)

[Longhorn](https://longhorn.io/)

[MinIO | High Performance, Kubernetes Native Object Storage](https://min.io/)

[gostor/awesome-go-storage: A curated list of awesome Go storage projects and libraries](https://github.com/gostor/awesome-go-storage)

[Auth.js](https://authjs.dev/)

[Kaggle: Your Machine Learning and Data Science Community](https://www.kaggle.com/)

[Pgweb - Cross-platform client for PostgreSQL databases](https://sosedoff.github.io/pgweb/)

[Grammarly: Free Online Writing Assistant](https://www.grammarly.com/)

[Ginger Software | English Grammar & Writing App](https://www.gingersoftware.com/)

[LanguageTool - Online Grammar, Style & Spell Checker](https://languagetool.org/)

[typopo—fix microtypography in multiple languages](https://typopo.org/)

[Vanilla OS](https://vanillaos.org/)

[Real-time Data Platform - Multi-model NoSQL | Aerospike](https://aerospike.com/)

[libSQL | libSQL is an open source, open contribution fork of SQLite. We aim to evolve it to suit many more use cases than SQLite was originally designed for.](https://libsql.org/)

[Introduction | golangci-lint](https://golangci-lint.run/)

[Greptime: Cloud-scale, Fast and Efficient Time Series Data Infrastructure | Greptime](https://greptime.com/)

[TED: Ideas Worth Spreading](https://www.ted.com/)

[Lessons Worth Sharing | TED-Ed](https://ed.ted.com/)

[Morning Brew](https://www.morningbrew.com/daily)

[Blinkist: Powerful ideas—15 minutes at a time](https://www.blinkist.com/)

[Listen to podcasts with the best free podcasting app - built by listeners, for listeners. | PocketCasts](https://pocketcasts.com/)

[Online Classes for Creatives | Skillshare](https://www.skillshare.com/en/)

[Online Courses - Learn Anything, On Your Schedule | Udemy](https://www.udemy.com/)

[Sololearn: Learn to Code](https://www.sololearn.com/)

[Khan Academy | Free Online Courses, Lessons & Practice](https://www.khanacademy.org/)

[Brilliant | Learn interactively](https://brilliant.org/)

[Learn to Code — For Free — Coding Courses for Busy People | freeCodeCamp](https://www.freecodecamp.org/)

[Your Career in Web Development Starts Here | The Odin Project](https://www.theodinproject.com/)

[CS50: Introduction to Computer Science | Harvard University](https://pll.harvard.edu/course/cs50-introduction-computer-science?delta=0)

[MIT OpenCourseWare | Free Online Course Materials](https://ocw.mit.edu/)

[edX | Free Online Courses by Harvard, MIT, & more | edX](https://www.edx.org/)

[Exercism](https://exercism.org/)

[MDN Web Docs](https://developer.mozilla.org/en-US/)

[Codewars - Achieve mastery through coding practice and developer mentorship](https://www.codewars.com/)

[Developer Roadmaps](https://roadmap.sh/)

[Hackertab | Developer news](https://hackertab.dev/)

[DigitalOcean Community | DigitalOcean](https://www.digitalocean.com/community)

[MongoDB Course Catalog Homepage | MongoDB University](https://learn.mongodb.com/catalog)

[Cockroach University | Cockroach Labs](https://www.cockroachlabs.com/cockroach-university/)

[DuckDB - An in-process SQL OLAP database management system](https://duckdb.org/)

[JanusGraph](https://janusgraph.org/)

[Ditto - Sync without Internet](https://www.ditto.live/)

[CozoDB: embedded Datalog, performant graphs](https://www.cozodb.org/)

[Hive Docs](https://docs.hivedb.dev/#/)

[RethinkDB: the open-source database for the realtime web](https://rethinkdb.com/)

[My Knowledge Wiki | Everything I know](https://wiki.nikiv.dev/)

[Hono - Ultrafast web framework for Cloudflare Workers, Deno, and Bun.](https://honojs.dev/)

[Find and fix problems in your JavaScript code - ESLint - Pluggable JavaScript Linter](https://eslint.org/)

[Prettier · Opinionated Code Formatter](https://prettier.io/)

[DevToys - A Swiss Army knife for developers](https://devtoys.app/)

[fresh - The next-gen web framework.](https://fresh.deno.dev/)

[GPRM : GitHub Profile ReadMe Maker](https://gprm.itsvg.in/)

[listmonk - Free and open source self-hosted newsletter, mailing list manager, and transactional mails](https://listmonk.app/)

[ekzhang/bore: 🕳 bore is a simple CLI tool for making tunnels to localhost](https://github.com/ekzhang/bore)

[charmbracelet/vhs: Your CLI home video recorder 📼](https://github.com/charmbracelet/vhs)

[orf/gping: Ping, but with a graph](https://github.com/orf/gping)

[Schniz/fnm: 🚀 Fast and simple Node.js version manager, built in Rust](https://github.com/Schniz/fnm)

[filhodanuvem/gitql: 💊 A git query language](https://github.com/filhodanuvem/gitql)

[markmap](https://markmap.js.org/)

[tldr pages](https://tldr.sh/)

[Uptime Kuma](https://uptime.kuma.pet/)

[Git for Data - lakeFS](https://lakefs.io/)

[Morgen: Unified calendar, scheduler and task manager](https://www.morgen.so/)

[The HTML presentation framework | reveal.js](https://revealjs.com/)

[impress.js | presentation tool based on the power of CSS3 transforms and transitions in modern browsers | by Bartek Szopka @bartaz](https://impress.js.org/#/bored)

[Marp: Markdown Presentation Ecosystem](https://marp.app/)

[Eleventy, a simpler static site generator](https://www.11ty.dev/)

[Push Notifications Made Simple | React Native Expo | Flutter | Native Notify](https://nativenotify.com/)

[RxJS](https://rxjs.dev/)

[Parabeac](https://parabeac.com/)

[Remotion | Make videos programmatically](https://www.remotion.dev/)

[React Query - Hooks for fetching, caching and updating asynchronous data in React](https://react-query-v3.tanstack.com/)

[docsify](https://docsify.js.org/#/)

[the native web](https://www.thenativeweb.io/)

[Yjs Shared Editing](https://yjs.dev/)

[Orbit.js - The Universal Data Layer | Orbit.js](https://orbitjs.com/)

[GitHub - primus/primus: Primus, the creator god of the transformers & an abstraction layer for real-time to prevent module lock-in.](https://github.com/primus/primus)

[GitHub - thevahidal/soul: 🕉 A SQLite REST and realtime server](https://github.com/thevahidal/soul)

[The web framework for perfectionists with deadlines | Django](https://www.djangoproject.com/)

[Floating UI - Create tooltips, popovers, dropdowns, and more](https://floating-ui.com/)

[Turf.js | Advanced geospatial analysis](https://turfjs.org/)

[Danfo.js Documentation - Danfo.js](https://danfo.jsdata.org/)

[Terraform by HashiCorp](https://www.terraform.io/)

[Pulumi - Universal Infrastructure as Code](https://www.pulumi.com/)

[osquery | Easily ask questions about your Linux, Windows, and macOS infrastructure](https://osquery.io/)

[API for Email, SMS, Web & Mobile Push Notifications | Courier](https://www.courier.com/)

[Orchest - Build data pipelines, the easy way.](https://www.orchest.io/)

[Open source APM | SigNoz](https://signoz.io/)

[Linen | Slack alternative designed for communities](https://www.linen.dev/)

[The Global Home for Platform Engineers](https://platformengineering.org/)

[tauri-apps/awesome-tauri: 🚀 Awesome Tauri Apps, Plugins and Resources](https://github.com/tauri-apps/awesome-tauri)

[Hello from Xplorer | Xplorer](https://xplorer.space/)

[Apache AGE Community](https://age.apache.org/)

[Confluent: Data in Motion](https://www.confluent.io/)

[Pagefind | Pagefind](https://pagefind.app/)

[What is Fuse.js? | Fuse.js](https://fusejs.io/)

[Load testing for engineering teams | Grafana k6](https://k6.io/)

[mateusfreira/nun-db: A realtime database written in rust](https://github.com/mateusfreira/nun-db)

[UpCloud - Effortless global cloud infrastructure for SMBs](https://upcloud.com/)

[The cloud of choice | Scaleway](https://www.scaleway.com/en/)

[Cloud computing, evolved | Paperspace](https://www.paperspace.com/)

[GPU Cloud, Workstations, Servers, Laptops for Deep Learning | Lambda](https://lambdalabs.com/)

[Rent GPUs | Vast.ai](https://vast.ai/)

[Rent Cloud GPUs from $0.2/hour | RunPod](https://www.runpod.io/)

[Golem Network](https://www.golem.network/)

[Internxt – World's Most Private Cloud Storage](https://internxt.com/)

[AlternativeTo - Crowdsourced software recommendations](https://alternativeto.net/)

[iLovePDF | Online PDF tools for PDF lovers](https://www.ilovepdf.com/)

[Safing Portmaster - Easy Privacy](https://safing.io/)

[Doppler | The #1 SecretOps Platform](https://www.doppler.com/)

[Home | Aurelia](https://aurelia.io/)

[Mermaid | Diagramming and charting tool](https://mermaid.js.org/#/)

[LaTeX - A document preparation system](https://www.latex-project.org/)

[KaTeX – The fastest math typesetting library for the web](https://katex.org/)

[AppMap | Runtime Code Analysis for Developers](https://appmap.io/)

[APITable | Open source no-code database and Airtable alternative](https://apitable.com/)

[roneli/fastgql: Blazing fast, instant realtime & extendable GraphQL APIs powered by gqlgen](https://github.com/roneli/fastgql)

[Use Azure Cosmos DB for Free](https://cosmos.azure.com/try/)

[NoSQL Database | RavenDB ACID NoSQL Document Database](https://ravendb.net/)

[ArangoDB, the database for graph and beyond](https://www.arangodb.com/)

[Home | OrientDB Community Edition](https://orientdb.org/)

[Apache CouchDB](https://couchdb.apache.org/)

[Apache Cassandra | Apache Cassandra Documentation](https://cassandra.apache.org/_/index.html)

[Graphite](https://graphiteapp.org/)

[TanStack | High Quality Open-Source Software for Web Developers](https://tanstack.com/)

[Axios](https://axios-http.com/)

[React Hooks for Data Fetching – SWR](https://swr.vercel.app/)

[Puppeteer | Puppeteer](https://pptr.dev/)

[A lightweight Node.js private proxy registry | Verdaccio](https://verdaccio.org/)

[Volta - The Hassle-Free JavaScript Tool Manager](https://volta.sh/)

[D3.js - Data-Driven Documents](https://d3js.org/)

[Chart.js | Open source HTML5 Charts for your website](https://www.chartjs.org/)

[Aerokube | Efficient Browser Automation Infrastructure](https://aerokube.com/)

[Crusher — Low-code, fast all-in-one testing framework](https://crusher.dev/)

[Welcome To Partytown](https://partytown.builder.io/)

[Pico.css • Minimal CSS Framework for semantic HTML](https://picocss.com/)

[OSS Database | Open source alternatives to popular software](https://ossdatabase.com/)

[Buffalo – Rapid Web Development in Go](https://gobuffalo.io/)

[Thin Backend: Instead of manually writing REST API endpoints or GraphQL resolvers you can use a Thin Backend server to automatically get a fully featured API backend.](https://thin.dev/)

[mgramin/awesome-db-tools: Everything that makes working with databases easier](https://github.com/mgramin/awesome-db-tools)

[Get Started - Workspace - Visual Studio Code](https://vscode.dev/)

[Coder - Remote development on your infrastructure](https://coder.com/)

[Membrane.](https://www.membrane.io/)

[Interval | The frontendless framework for high growth companies](https://interval.com/)

[API Design & Documentation Management | Designing & Building OpenAPIs | Stoplight](https://stoplight.io/)

[Crossnote](https://crossnote.app/)

[Superlist](https://www.superlist.com/)

[HackerRank](https://www.hackerrank.com/)

[Coding Games and Programming Challenges to Code Better](https://www.codingame.com/start)

[GeeksforGeeks | A computer science portal for geeks](https://www.geeksforgeeks.org/)

[Competitive Programming | Participate & Learn | CodeChef](https://www.codechef.com/)

[W3Schools Online Web Tutorials](https://www.w3schools.com/)

[Coursera | Degrees, Certificates, & Free Online Courses](https://www.coursera.org/)

[Learn the Latest Tech Skills; Advance Your Career | Udacity](https://www.udacity.com/)

[GitHub - bitnine-oss/agensgraph: AgensGraph, a transactional graph database based on PostgreSQL](https://github.com/bitnine-oss/agensgraph)

[ArcadeDB - The Next Generation Multi-Model DBMS](https://arcadedb.com/)

[Approzium | Approzium](https://approzium.com/)

[FusionDB](https://www.fusiondb.com/)

[CovenantSQL - The Blockchain SQL Database](https://covenantsql.io/)

[StarRocks | A High-Performance Analytical Database](https://www.starrocks.io/)

[LiteDB :: A .NET embedded NoSQL database](https://www.litedb.org/)

[GitHub - SapphireDb/SapphireDb: SapphireDb Server, a self-hosted, easy to use realtime database for Asp.Net Core and EF Core](https://github.com/SapphireDb/SapphireDb)

[GUN — the database for freedom fighters - Docs v2.0](https://gun.eco/)

[YoMo](https://yomo.run/)

[GitHub - miroslavpejic85/mirotalksfu: 🏆 WebRTC - SFU - Simple, Secure, Scalable Real-Time Video Conferences Up to 4k, compatible with all browsers and platforms.](https://github.com/miroslavpejic85/mirotalksfu)

[GitHub - dsuryd/dotNetify: Simple, lightweight, yet powerful way to build real-time web apps.](https://github.com/dsuryd/dotNetify)

[dotNetify](https://dotnetify.net/)

[Iron.io - Hosted Serverless Tools](https://www.iron.io/)

[Radio4000](https://radio4000.com/)

[Hello from Harmonoid | Harmonoid](https://harmonoid.com/)

[Spotube](https://spotube.netlify.app/)

[BlackHole](https://sangwan5688.github.io/)

[muffon - Music streaming browser](https://muffon.netlify.app/)

[Nuclear Music Player](https://nuclear.js.org/)

[Audius](https://audius.co/)

[Spotify – Web Player](https://open.spotify.com/?)

[Platform - Bloom AI](https://bloomai.co/)

[DeepMind](https://www.deepmind.com/)

[ChatGPT: Optimizing Language Models for Dialogue](https://openai.com/blog/chatgpt/)

[BigchainDB • • The blockchain database.](https://www.bigchaindb.com/)

[Pushpin | Reverse proxy for realtime APIs](https://pushpin.org/)

[Tailscan - Browser inspector for Tailwind CSS](https://tailscan.com/)

[Wallaby - Immediate JavaScript test feedback in your IDE as-you-type](https://wallabyjs.com/)

[Console log output right next to your code](https://console-ninja.com/)

[SashiDo.io | API Development, Deployment and Scaling made simple.](https://www.sashido.io/)

[Canonic | The back to your frontend](https://canonic.dev/)

[PieSocket Realtime - Scalable WebSockets API and SDKs](https://www.piesocket.com/)

[Nchan - flexible pubsub for the modern web](https://nchan.io/)

[Home - ChilliCream GraphQL Platform](https://chillicream.com/)

[Spring | Home](https://spring.io/)

[Northflank — Deploy any code, job, or database in seconds.](https://northflank.com/)

[Couchbase: Best NoSQL Cloud Database Service](https://www.couchbase.com/)

[Bitnami: Packaged Applications for Any Platform - Cloud, Container, Virtual Machine](https://bitnami.com/)

[SmartDB - Serverless MySQL made easy](https://smartdb.io/)

[Managed Open-Source Applications on Kubernetes Platform](https://www.cloudclusters.io/)

[IBM Cloud | IBM](https://www.ibm.com/cloud)

[CrateDB – Distributed SQL Database Enabling Data Insights at Scale](https://crate.io/)

[StackGres | Enterprise Postgres made easy. On Kubernetes.](https://stackgres.io/)

[Percona – The Database Performance Experts](https://www.percona.com/)

[EDB: Open-Source, Enterprise Postgres Database Management](https://www.enterprisedb.com/)

[Warrant | Authorization & Access Control as a Service](https://warrant.dev/)

[MojoAuth | Secure and Delightful Passwordless Login Solution](https://mojoauth.com/)

[PropelAuth - Authentication for B2B products](https://www.propelauth.com/)

[The Liquibase Community | The Database DevOps Community](https://www.liquibase.org/)

[Rapid Spring Boot Prototypes | Bootify.io](https://bootify.io/)

[Greenplum Database](https://greenplum.org/)

[OrioleDB – the next generation storage engine for PostgreSQL](https://www.orioledata.com/)

[Faker | Faker](https://fakerjs.dev/)

[Database Management and Hosting | ObjectRocket](https://www.objectrocket.com/)

[Database Hosting - Fully Managed Cloud Services At ScaleGrid](https://scalegrid.io/)

[Hello from Rest | Rest](https://rest-go.com/)

[ForetagInc/alchemy: \[WIP\] High performance, realtime BaaS with RBAC, graphing, full text search, S3 / B2 Storage and GIS with a GraphQL, gRPC and REST API](https://github.com/ForetagInc/alchemy)

[ksqlDB: The database purpose-built for stream processing applications.](https://ksqldb.io/)

[Ktor: Build Asynchronous Servers and Clients in Kotlin | Ktor Framework](https://ktor.io/)

[ASP.NET | Open-source web framework for .NET](https://dotnet.microsoft.com/en-us/apps/aspnet)

[Real-time ASP.NET with SignalR | .NET](https://dotnet.microsoft.com/en-us/apps/aspnet/signalr)

[Vapor](https://vapor.codes/)

[JetBrains/xodus: Transactional schema-less embedded database used by JetBrains YouTrack and JetBrains Hub.](https://github.com/JetBrains/xodus)

[Argo CD - Declarative GitOps CD for Kubernetes](https://argo-cd.readthedocs.io/en/stable/)

[Istio](https://istio.io/)

[Home | Telepresence](https://www.telepresence.io/)

[Apache Spark™ - Unified Engine for large-scale data analytics](https://spark.apache.org/)

[Apache Flink: Stateful Computations over Data Streams](https://flink.apache.org/)

[Apache Storm](https://storm.apache.org/)

[Messaging that just works — RabbitMQ](https://www.rabbitmq.com/)

[Apache Kafka](https://kafka.apache.org/)

[Druid | Database for modern analytics applications](https://druid.apache.org/)

[Apache HBase – Apache HBase™ Home](https://hbase.apache.org/)

[API Platform: The API-first framework (REST, GraphQL and real-time)](https://api-platform.com/)

[Vector database - Milvus](https://milvus.io/)

[Home | GraphQLite](https://graphqlite.com/)

[Home Page | Pal.js Start your NodeJs, Prisma, GraphQL, React project](https://paljs.com/)

[GitHub - EntityGraphQL/EntityGraphQL: A GraphQL library for .NET](https://github.com/EntityGraphQL/EntityGraphQL)

[GitHub - movio/bramble: The Movio GraphQL Gateway](https://github.com/movio/bramble)

[GitHub - nautilus/gateway: A federated api gateway for graphql services. https://gateway.nautilus.dev/](https://github.com/nautilus/gateway)

[GitHub - daptin/daptin: Daptin - Backend As A Service - GraphQL/JSON-API Headless CMS](https://github.com/daptin/daptin)

[Pragma: Build Beautiful GraphQL APIs In No Time | Pragma](https://pragmalang.com/)

[GitHub - dunglas/vulcain: Fast and idiomatic client-driven REST APIs.](https://github.com/dunglas/vulcain)

[K8ssandra - K8ssandra, Apache Cassandra® on Kubernetes](https://k8ssandra.io/)

[TheHive Project](https://thehive-project.org/)

[Sylius - Open Source Headless eCommerce Platform](https://sylius.com/)

[Open Source ECOMMERCE | GRANDNODE](https://grandnode.com/)

[Kubeshark — API Traffic viewer for Kubernetes](https://kubeshark.co/)

[Home - iceoryx.io](https://iceoryx.io/v2.0.2/)

[Orchestrating Pulsar Functions for Serverless Event Streaming | Function Mesh](https://functionmesh.io/)

[KEDA | Kubernetes Event-driven Autoscaling](https://keda.sh/)

[solo-io/gloo: The Feature-rich, Kubernetes-native, Next-Generation API Gateway Built on Envoy](https://github.com/solo-io/gloo)

[Solo.io - The Service Mesh and API Platform for Kubernetes, Zero-Trust, Microservices](https://www.solo.io/)

[Cloud Custodian](https://cloudcustodian.io/)

[Vector | A lightweight, ultra-fast tool for building observability pipelines](https://vector.dev/)

[Home | NodeBB | Your Community Forum Platform](https://nodebb.org/)

[buraksezer/olric: Distributed in-memory object store. It can be used both as an embedded Go library and as a language-independent service.](https://github.com/buraksezer/olric)

[Bazel at Enterprise Scale | BuildBuddy](https://www.buildbuddy.io/)

[Bazel](https://bazel.build/)

[The most powerful real-time 3D creation tool - Unreal Engine](https://www.unrealengine.com/en-US)

[CRYENGINE | The complete solution for next generation game development by Crytek](https://www.cryengine.com/)

[Unity Real-Time Development Platform | 3D, 2D VR & AR Engine](https://unity.com/)

[Bevy - A data-driven game engine built in Rust](https://bevyengine.org/)

[Amethyst - The open source, data-driven game engine](https://amethyst.rs/)

[Godot Engine - Free and open source 2D and 3D game engine](https://godotengine.org/)

[Stride Game Engine](https://www.stride3d.net/)

[Ct.js — a free game editor](https://ctjs.rocks/)

[Free and Easy Game-Making App | GDevelop](https://gdevelop.io/)

[Defold - Official Homepage - Cross platform game engine](https://defold.com/)

[Flame Engine](https://flame-engine.org/)

[blender.org - Home of the Blender project - Free and Open 3D Creation Software](https://www.blender.org/)

[Krita | Digital Painting. Creative Freedom.](https://krita.org/en/)

[Kdenlive - Video Editing Freedom](https://kdenlive.org/en/)

[darktable](https://www.darktable.org/)

[Affinity – Professional Creative Software](https://affinity.serif.com/en-us/)

[DaVinci Resolve 18 | Blackmagic Design](https://www.blackmagicdesign.com/products/davinciresolve)

[Encryption and data protection for your apps | Cossack Labs](https://www.cossacklabs.com/)

[@databases · Query SQL Databases using Node.js and TypeScript](https://www.atdatabases.org/)

[SheetJS - Home](https://sheetjs.com/)

[Open Assistant | Open Assistant](https://projects.laion.ai/Open-Assistant/)

[LocalSend](https://localsend.org/)

[IvanGoncharov/graphql-voyager: 🛰️ Represent any GraphQL API as an interactive graph](https://github.com/IvanGoncharov/graphql-voyager)

[GitHub - m1k1o/neko: A self hosted virtual browser that runs in docker and uses WebRTC.](https://github.com/m1k1o/neko)

[Home | Morpheus GraphQL](https://morpheusgraphql.com/)

[Ariadne · Python GraphQL Schema-first](https://ariadnegraphql.org/)

[Hello from GraphQL Java | GraphQL Java](https://www.graphql-java.com/)

[GraphQL CMS, Headless Content Management | Slicknode](https://slicknode.com/)

[Uno Platform - Build Pixel Perfect Applications](https://platform.uno/)

[Home | Isar Database](https://isar.dev/)

[Hyper™](https://hyper.is/)

[Starship: Cross-Shell Prompt](https://starship.rs/)

[Xterm.js](https://xtermjs.org/)

[Tile38 · Ultra Fast Geospatial Database & Geofencing Server](https://tile38.com/)

[immudb - Immutable Database](https://immudb.io/)

[Encore — Escape your backend development burdens](https://encore.dev/)

[FeatureBase](https://www.featurebase.com/)

[Database branching for any Postgres DB · Postgres.ai · Database Lab Engine (DLE)](https://postgres.ai/)

[Databunker secure db for ✔️ critical records ✔️ PII ✔️ PHI](https://databunker.org/)

[JuiceFS - Open Source Distributed POSIX File System for Cloud](https://juicefs.com/en/)

[cachegrand](https://cachegrand.io/)

[nutsdb/nutsdb: A simple, fast, embeddable, persistent key/value store written in pure Go. It supports fully serializable transactions and many data structures such as list, set, sorted set.](https://github.com/nutsdb/nutsdb)

[hibiken/asynq: Simple, reliable, and efficient distributed task queue in Go](https://github.com/hibiken/asynq)

[Flow Launcher](https://www.flowlauncher.com/)

[tw93/Pake: 🤱🏻 Turn any webpage into a desktop app with Rust. 🤱🏻 很简单的用 Rust 打包网页生成很小的桌面 App](https://github.com/tw93/Pake)

[GitHub - ionorg/ion: Real-Distributed RTC System by pure Go and Flutter](https://github.com/ionorg/ion)

[Home](https://screego.net/#/)

[SRS (Simple Realtime Server) | SRS](https://ossrs.io/lts/en-us/)

[Free Video Conferencing Software for Web & Mobile | Jitsi](https://jitsi.org/)

[Owncast - Free and Open Source Livestreaming](https://owncast.online/)

[mediasoup](https://mediasoup.org/)

[Guilded - Chat for Gaming Communities](https://www.guilded.gg/)

[MiroTalk a Free Secure Video Calls, Chat & Screen Sharing.](https://p2p.mirotalk.com/)

[GitHub - robertodoering/harpy: a Twitter app built with Flutter 🦅](https://github.com/robertodoering/harpy)

[The Most Advanced WYSIWYG Editor | Trusted Rich Text Editor | TinyMCE](https://www.tiny.cloud/)

[New Relic | Monitor, Debug and Improve Your Entire Stack](https://newrelic.com/)

[October - PHP CMS platform based on the Laravel Framework](https://octobercms.com/)

[Build Laravel Admin Panels - Fast :: Backpack for Laravel](https://backpackforlaravel.com/)

[Userbase: Logins and persistence for static sites](https://userbase.com/)

[maxpert/marmot: A distributed SQLite replicator built on top of NATS](https://github.com/maxpert/marmot)

[Payload CMS | Node & React TypeScript Headless CMS](https://payloadcms.com/)

[eko/authz: 🛡️ Authorization backend that comes with a UI for RBAC and ABAC permissions](https://github.com/eko/authz)

[Buttercup](https://buttercup.pw/)

[Infra — Modern infrastructure access](https://infrahq.com/)

[API Management Platform & API Gateway | Tyk.io](https://tyk.io/)

[buzzfeed/sso: sso, aka S.S.Octopus, aka octoboi, is a single sign-on solution for securing internal services](https://github.com/buzzfeed/sso)

[kanidm/kanidm: Kanidm: A simple, secure and fast identity management platform](https://github.com/kanidm/kanidm)

[Circuit Diagram - A Circuit Diagram Maker](https://www.circuit-diagram.org/)

[Analytics for Decision-Making | HEAVY.AI](https://www.heavy.ai/)

[Hello from Apache Pulsar | Apache Pulsar](https://pulsar.apache.org/)

[Apache ShardingSphere](https://shardingsphere.apache.org/)

[Apache Airflow](https://airflow.apache.org/)

[Welcome | Superset](https://superset.apache.org/)

[Apache ECharts](https://echarts.apache.org/en/index.html)

[Centrifugo – scalable real-time messaging server in a language-agnostic way. Set up once and forever. | Centrifugo](https://centrifugal.dev/)

[ntfy.sh | Send push notifications to your phone via PUT/POST](https://ntfy.sh/)

[Novu - The open-source notification infrastructure](https://novu.co/)

[Pink Design](https://pink.appwrite.io/)

[mirrord | mirrord](https://mirrord.dev/)

[Robyn - An async Python Framework written in Rust.](https://robyn.tech/)

[sourcegraph/conc: Better structured concurrency for go](https://github.com/sourcegraph/conc)

[The Lura Project](https://luraproject.org/)

[Semgrep — Find bugs and enforce code standards](https://semgrep.dev/)

[Thanos - Highly available Prometheus setup with long term storage capabilities](https://thanos.io/)

[Teleport: Identity-Native Infrastructure Access. Faster. More Secure.](https://goteleport.com/)

[Container management software for Docker, Kubernetes & Nomad](https://www.portainer.io/)

[Enterprise Kubernetes Management | Rancher](https://www.rancher.com/)

[Home](https://kurzgesagt.org/)

[Melodysheep](https://www.melodysheep.com/)

[Honeypot | Europe's Developer-Focused Job Platform](https://www.honeypot.io/)

[Hire the World’s Most Deeply Vetted Remote Developers | Turing](https://www.turing.com/)

[Cenit IO | integration Platform](https://web.cenit.io/)

[sql.js](https://sql.js.org/#/)

[GitHub - nalgeon/sqlean: The ultimate set of SQLite extensions](https://github.com/nalgeon/sqlean)

[ReadySet | Same database, (much) faster queries](https://readyset.io/)

[MikroORM: TypeScript ORM for Node.js based on Data Mapper, Unit of Work and Identity Map patterns. | MikroORM](https://mikro-orm.io/)

[Introduction | Replibyte](https://www.replibyte.com/docs/introduction/)

[AdminJS - the leading open-source admin panel for Node.js apps | AdminJS](https://adminjs.co/)

[Markdoc | A powerful, flexible, Markdown-based authoring framework](https://markdoc.dev/)

[File Upload Service for Web Apps – Storage & CDN Included](https://upload.io/)

[Theneo: High-Quality & Interactive API Docs](https://www.theneo.io/)

[Superblocks | Save 100s of developer hours on internal tools](https://www.superblocks.com/)

[Ddosify | No Code Distributed Observability Platform | Performance Impact Analysis Software](https://ddosify.com/)

[ruilisi/fortune-sheet: A drop-in javascript spreadsheet library that provides rich features like Excel and Google Sheets](https://github.com/ruilisi/fortune-sheet)

[Hoppscotch • Open source API development ecosystem • Hoppscotch](https://hoppscotch.io/)

[Podman](https://podman.io/)

[Buildah | buildah.io](https://buildah.io/)

[Podman Desktop - Containers and Kubernetes | Podman Desktop](https://podman-desktop.io/)

[Home | Task](https://taskfile.dev/)

[fzyzcjy/flutter\_rust\_bridge: High-level memory-safe binding generator for Flutter/Dart <-> Rust](https://github.com/fzyzcjy/flutter_rust_bridge)

[Spree Commerce - a headless open-source ecommerce platform](https://spreecommerce.org/)

[Vue.js - The Progressive JavaScript Framework | Vue.js](https://vuejs.org/)

[Angular](https://angular.io/)

[Preact | Preact: Fast 3kb React alternative with the same ES6 API. Components & Virtual DOM.](https://preactjs.com/)

[Svelte • Cybernetically enhanced web apps](https://svelte.dev/)

[Modern Site Generator for Vue.js - Gridsome](https://gridsome.org/)

[Swift.org - Welcome to Swift.org](https://www.swift.org/)

[Kotlin Programming Language](https://kotlinlang.org/)

[The Go Programming Language](https://go.dev/)

[Rust Programming Language](https://www.rust-lang.org/)

[The V Programming Language](https://vlang.io/)

[The Crystal Programming Language](https://crystal-lang.org/)

[The Scala Programming Language](https://www.scala-lang.org/)

[TypeScript: JavaScript With Syntax For Types.](https://www.typescriptlang.org/)

[Anaconda | The World's Most Popular Data Science Platform](https://www.anaconda.com/)

[.NET | Free. Cross-platform. Open Source.](https://dotnet.microsoft.com/en-us/)

[GitHub - pmndrs/zustand: 🐻 Bear necessities for state management in React](https://github.com/pmndrs/zustand)

[GitHub - pmndrs/jotai: 👻 Primitive and flexible state management for React](https://github.com/pmndrs/jotai)

[GitHub - Mimetis/Dotmim.Sync: A brand new database synchronization framework, multi platform, multi databases, developed on top of .Net Standard 2.0. https://dotmimsync.readthedocs.io/](https://github.com/Mimetis/Dotmim.Sync)

[refine | Build your React-based CRUD applications, without constraints! | refine](https://refine.dev/)

[🎉 Online Database designer for web developers.](https://dynobird.com/)

[pgModeler - PostgreSQL Database Modeler](https://pgmodeler.io/)

[Tailwind CSS Cheat Sheet](https://tailwindcomponents.com/cheatsheet/)

[Tailwind CSS Components - 600+ Free Examples and Templates](https://tailwindcomponents.com/)

[styled-components](https://styled-components.com/)

[The Fastest Frontend for the Headless Web | Gatsby](https://www.gatsbyjs.com/)

[Automerge CRDT | Automerge CRDT](https://automerge.org/)

[GitHub - GoogleChrome/workbox: 📦 Workbox: JavaScript libraries for Progressive Web Apps](https://github.com/GoogleChrome/workbox)

[GitHub - MobileNativeFoundation/Store: A Kotlin Multiplatform Library for Building Network-Resilient Applications](https://github.com/MobileNativeFoundation/Store)

[GitHub - crossbeam-rs/crossbeam: Tools for concurrent programming in Rust](https://github.com/crossbeam-rs/crossbeam)

[Dexie.js - Minimalistic IndexedDB Wrapper](https://dexie.org/)

[GitHub - tekartik/sembast.dart: Simple io database](https://github.com/tekartik/sembast.dart)

[GitHub - tekartik/sqflite: SQLite flutter plugin](https://github.com/tekartik/sqflite)

[GitHub - simolus3/drift: Drift is an easy to use, reactive, typesafe persistence library for Dart & Flutter.](https://github.com/simolus3/drift)

[GitHub - pinchbv/floor: The typesafe, reactive, and lightweight SQLite abstraction for your Flutter applications](https://github.com/pinchbv/floor)

[GitHub - neon-bindings/neon: Rust bindings for writing safe and fast native Node.js modules.](https://github.com/neon-bindings/neon)

[The AI Search Engine You Control | AI Chat & Apps](https://you.com/)

[Graph Database: Open Source & Distributed | NebulaGraph](https://www.nebula-graph.io/)

[HugeGraph](https://hugegraph.apache.org/)

[GitHub - krotik/eliasdb: EliasDB a graph-based database.](https://github.com/krotik/eliasdb)

[GitHub - superinsight/superinsight-db: Relational Database for Unstructured Data](https://github.com/superinsight/superinsight-db)

[Berty · Berty Technologies](https://berty.tech/)

[Fluid Framework](https://fluidframework.com/)

[Hazelcast Jet · Open-Source Distributed Stream Processing](https://jet-start.sh/)

[RustDesk | Open source remote desktop software | The Open Source TeamViewer Alternative](https://rustdesk.com/)

[rwynn/monstache: a go daemon that syncs MongoDB to Elasticsearch in realtime. you know, for search.](https://github.com/rwynn/monstache)

[resgate.io](https://resgate.io/)

[MiroTalk SFU, Free Video Calls, Messaging and Screen Sharing](https://sfu.mirotalk.com/)

[Parabol | Free Agile Meeting Tool for Remote Teams](https://www.parabol.co/)

[OSS Insight](https://ossinsight.io/)

[ERDiagram playground](https://erdiagram.nestorrente.com/)

[AppsCode Inc.](https://appscode.com/)

[Dropzone.js](https://www.dropzone.dev/)

[Grid.js - Advanced JavaScript table plugin](https://gridjs.io/)

[Leaflet - a JavaScript library for interactive maps](https://leafletjs.com/)

[deck.gl](https://deck.gl/)

[Iconify - all open source icons in one framework](https://iconify.design/)

[remorses/genql: Type safe TypeScript client for any GraphQL API](https://github.com/remorses/genql)

[github/gh-ost: GitHub's Online Schema-migration Tool for MySQL](https://github.com/github/gh-ost)

[Apache Mesos](https://mesos.apache.org/)

[Apache OpenWhisk is a serverless, open source cloud platform](https://openwhisk.apache.org/)

[Transform](https://transform.tools/)

[Altair GraphQL Client](https://altairgraphql.dev/)

[App-vNext/Polly: Polly is a .NET resilience and transient-fault-handling library that allows developers to express policies such as Retry, Circuit Breaker, Timeout, Bulkhead Isolation, and Fallback in a fluent and thread-safe manner. From version 6.0.1, Polly targets .NET Standard 1.1 and 2.0+.](https://github.com/App-vNext/Polly)

[gnet - A high-performance, lightweight, non-blocking, event-driven networking framework written in pure Go | gnet](https://gnet.host/)

[Trigger.dev - Effortless automation built for developers](https://trigger.dev/)

[MediaCMS.io](https://mediacms.io/)

[PixiJS](https://pixijs.com/)

[valeriansaliou/sonic: 🦔 Fast, lightweight & schema-less search backend. An alternative to Elasticsearch that runs on a few MBs of RAM.](https://github.com/valeriansaliou/sonic)

[Heroic Games Launcher](https://heroicgameslauncher.com/)

[Playnite - video game library manager](https://playnite.link/)

[Vespa - the big data serving engine](https://vespa.ai/)

[Marqo | Tensor-based Search and Analytics engine](https://www.marqo.ai/)

[effector | effector](https://effector.dev/)

[egg - Born to build better enterprise frameworks and apps - Egg](https://www.eggjs.org/)

[Cider](https://cider.sh/)

[DBeaver PRO | One tool for all data sources](https://dbeaver.com/)

[Squidex/squidex: Headless CMS and Content Managment Hub](https://github.com/squidex/squidex)

[mixcore/mix.core: 🚀 Mixcore CMS is an open-source CMS that supports both headless and decoupled to easily build any kinds of app/web app/all in all/customizable APIs built on top of ASP.NET Core / Dotnet Core. It is a completely open-source ASP.NET Core (Dotnet Core) CMS solution. https://mixcore.org](https://github.com/mixcore/mix.core)

[Redocly/redoc: 📘 OpenAPI/Swagger-generated API Reference Documentation](https://github.com/Redocly/redoc)

[Camberi/firecms: Awesome Firebase/Firestore-based CMS. The missing admin panel for your Firebase project!](https://github.com/Camberi/firecms)

[magidoc-org/magidoc: Autogenerate static GraphQL API documentation](https://github.com/magidoc-org/magidoc)

[spica-engine/spica: Spica is a development engine to build fast & efficient applications.](https://github.com/spica-engine/spica)

[estruyf/vscode-front-matter: Front Matter is a CMS running straight in Visual Studio Code. Can be used with static site generators like Hugo, Jekyll, Hexo, NextJs, Gatsby, and many more...](https://github.com/estruyf/vscode-front-matter)

[Tahoma2D - Free 2D & stop motion animation software](https://tahoma2d.org/)

[Storybook: Frontend workshop for UI development](https://storybook.js.org/)

[GitHub - nativefier/nativefier: Make any web page a desktop application](https://github.com/nativefier/nativefier)

[Docz](https://www.docz.site/)

[Theme UI](https://theme-ui.com/)

[Taiga: Your opensource agile project management software](https://www.taiga.io/)

[GitHub - rest-go/rest: Rest serves a fully RESTful API from any SQL database](https://github.com/rest-go/rest)

[GitHub - aquametalabs/aquameta: Web development platform built entirely in PostgreSQL](https://github.com/aquametalabs/aquameta)

[GMass: Mail merge and mass emails for Gmail](https://www.gmass.co/)

[Midjourney](https://midjourney.com/home/?callbackUrl=%2Fapp%2F)

[Stable Diffusion Online](https://stablediffusionweb.com/)

[Tenacity](https://tenacityaudio.org/)

[ThreeDotsLabs/watermill: Building event-driven applications the easy way in Go.](https://github.com/ThreeDotsLabs/watermill/)

[nsqio/nsq: A realtime distributed messaging platform](https://github.com/nsqio/nsq)

[Codiga: Static Code Analysis in Real-Time](https://www.codiga.io/)

[massCode | A free and open source code snippets manager for developers](https://masscode.io/)

[Fast and reliable end-to-end testing for modern web apps | Playwright](https://playwright.dev/)

[kataras/iris: The fastest HTTP/2 Go Web Framework. New, modern, easy to learn. Fast development with Code you control. Unbeatable cost-performance ratio | 谢谢 | #golang](https://github.com/kataras/iris)

[OpenAI](https://openai.com/)

[Hugging Face – The AI community building the future.](https://huggingface.co/)

[GitHub - diogob/postgres-websockets: PostgreSQL + Websockets](https://github.com/diogob/postgres-websockets)

[GitHub - zombodb/zombodb: Making Postgres and Elasticsearch work together like it's 2022](https://github.com/zombodb/zombodb)

[motion-canvas/motion-canvas: Visualize Complex Ideas Programmatically](https://github.com/motion-canvas/motion-canvas)

[D2 Tour | D2 Documentation](https://d2lang.com/tour/intro/)

[Redis](https://redis.io/)

[amacneil/dbmate: A lightweight, framework-agnostic database migration tool.](https://github.com/amacneil/dbmate)

[Avalonia UI - Home](https://avaloniaui.net/)

[Plural | Deploy, maintain, and scale the open-source applications you love](https://www.plural.sh/)

[Noted - Self Hosted App Reviews](https://noted.lol/)

[RippleUI | TailwindCSS Components](https://www.ripple-ui.com/?ref=producthunt)

[Hub Promyze](https://bestcodingpractices.dev/)

[Stashpad - A new way to take notes](https://www.stashpad.com/)

[It's FOSS](https://itsfoss.com/)

[Stack Diary - For Developers, Designers and Makers.](https://stackdiary.com/)

[DZone: Programming & DevOps news, tutorials & tools](https://dzone.com/)

[Mamba UI | Free HTML components and templates built with Tailwind CSS](https://mambaui.com/)

[Headless UI - Unstyled, fully accessible UI components](https://headlessui.com/)

[500+ TailwindCSS components, free for commercial use in Tailwind Elements](https://tailwind-elements.com/)

[Xtend UI - Xtend UI is a powerful frontend library of Tailwind CSS components enhanced by vanilla js. It helps you build interfaces with advanced interactions and animations.](https://xtendui.com/)

[Tailwind UI Kit - Tailwind CSS components and templates](https://tailwinduikit.com/)

[Meraki UI Free Tailwind CSS Components](https://merakiui.com/)

[Tailblocks — Ready-to-use Tailwind CSS blocks](https://tailblocks.cc/)

[Free Open Source Tailwind CSS Components | HyperUI](https://www.hyperui.dev/)

[Kimia-UI - UI Components for React built with Tailwind CSS 3](https://kimia-ui.vercel.app/)

[Tailwind Starter Kit | Free Components Library for TailwindCSS](https://www.creative-tim.com/learning-lab/tailwind-starter-kit/presentation)

[Material Tailwind - Easy-to-use Tailwind CSS components library with Material Design](https://www.material-tailwind.com/)

[Konsta UI - Mobile UI components built with Tailwind CSS](https://konstaui.com/)

[Preline UI, crafted with Tailwind CSS](https://preline.co/)

[Vector Icons and Stickers - PNG, SVG, EPS, PSD and CSS](https://www.flaticon.com/)

[Font Awesome](https://fontawesome.com/)

[7,200,000+ free and premium vector icons, illustrations and 3D illustrations](https://www.iconfinder.com/)

[Free Vectors, Stock Photos & PSD Downloads | Freepik](https://www.freepik.com/)

[Download Free Vector Art, Stock Photos & Stock Video Footage](https://www.vecteezy.com/)

[Browse Fonts - Google Fonts](https://fonts.google.com/?selected=Material+Icons)

[NvChad · Neovim with lua is cool](https://nvchad.com/)

[Homepage - Flyway](https://flywaydb.org/)

[EmailJS](https://www.emailjs.com/)

[Can I use... Support tables for HTML5, CSS3, etc](https://caniuse.com/)

[CSS-Tricks - Tips, Tricks, and Techniques on using Cascading Style Sheets.](https://css-tricks.com/)

[Bundlephobia | Size of npm dependencies](https://bundlephobia.com/)

[Font Flipper - Find Free Fonts](https://fontflipper.com/upload)

[Awwwards - Website Awards - Best Web Design Trends](https://www.awwwards.com/)

[Dribbble - Discover the World’s Top Designers & Creative Professionals](https://dribbble.com/)

[Lazyweb App](https://app.lazyweb.rocks/)

[Tinkercad | Create 3D digital designs with online CAD | Tinkercad](https://www.tinkercad.com/)

[Focalboard: Open source alternative to Trello, Asana, and Notion](https://www.focalboard.com/)

[The open digital workplace | TWAKE](https://twake.app/)

[SqlKata the dotnet SQL query builder](https://sqlkata.com/)

[Anytype.io](https://anytype.io/)

[Qatalog | Your Bespoke Operating System for Work](https://qatalog.com/)

[Flutter Awesome](https://flutterawesome.com/)

[Restack - Unlimited open source products — Restack](https://www.restack.io/)

[Data Version Control · DVC](https://dvc.org/)

[Unleash: Open-Source Feature Flag Management](https://www.getunleash.io/)

[Easily run Windows software on Linux with Bottles! ⋅ Bottles](https://usebottles.com/)

[The starting point of your new website | Boltcms.io](https://boltcms.io/)

[GitHub - go-jet/jet: Type safe SQL builder with code generation and automatic query result data mapping](https://github.com/go-jet/jet)

[vlcn.io - Device Sync & Collaboration Simplified](https://vlcn.io/)

[GitHub - vlcn-io/cr-sqlite: Convergent, Replicated SQLite. Multi-writer and CRDT support for SQLite](https://github.com/vlcn-io/cr-sqlite)

[GitHub - lana-k/sqliteviz: Instant offline SQL-powered data visualisation in your browser](https://github.com/lana-k/sqliteviz)

[Descript | All-in-one video editing, as easy as a doc.](https://www.descript.com/)

[tus - resumable file uploads](https://tus.io/)

[Webhook.site - Test, process and transform emails and HTTP requests](https://webhook.site/)

[Earthly - Better Builds](https://earthly.dev/)

[leangen/graphql-spqr: Build a GraphQL service in seconds](https://github.com/leangen/graphql-spqr)

[ExpediaGroup/graphql-kotlin: Libraries for running GraphQL in Kotlin](https://github.com/ExpediaGroup/graphql-kotlin)

[ReactiveX](https://reactivex.io/)

[Garage - An open-source distributed object storage service](https://garagehq.deuxfleurs.fr/)

[LIFTBRIDGE · Lightweight, fault-tolerant message streams](https://liftbridge.io/)

[Agora Real-Time Voice and Video Engagement](https://www.agora.io/en/)

[Chat API & In-App Messaging as a Service - Best Real-time Platform](https://getstream.io/chat/)

[Ballerina Home](https://ballerina.io/)

[atomicdata-dev/atomic-data-rust: Create, share, fetch and model Atomic Data! This project consists of a graph database + server, a CLI and a rust library.](https://github.com/atomicdata-dev/atomic-data-rust)

[Raytha | Open source .NET CMS](https://raytha.com/)

[https://www.wpgraphql.com](https://www.wpgraphql.com/)

[Ceph.io — Home](https://ceph.io/en/)

[seaweedfs/seaweedfs: SeaweedFS is a fast distributed storage system for blobs, objects, files, and data lake, for billions of files! Blob store has O(1) disk seek, cloud tiering. Filer supports Cloud Drive, cross-DC active-active replication, Kubernetes, POSIX FUSE mount, S3 API, S3 Gateway, Hadoop, WebDAV, encryption, Erasure Coding.](https://github.com/seaweedfs/seaweedfs)

[BullMQ - Background Jobs processing and message queue for NodeJS | BullMQ](https://bullmq.io/)

[Airplane | Developer infrastructure for internal tools](https://www.airplane.dev/)

[Localtonet | Localhost to Internet](https://localtonet.com/)

[Alpaca - Developer-First API for Trading](https://alpaca.markets/)

[Home - Estuary](https://www.estuary.dev/)

[The ReScript Programming Language](https://rescript-lang.org/)

[dnd kit – a modern drag and drop toolkit for React](https://dndkit.com/)

[AutoAnimate - Add motion to your apps with a single line of code](https://auto-animate.formkit.com/)

[Cron | Early access](https://cron.com/)

[webhookthing](https://webhookthing.com/)

[npm](https://www.npmjs.com/)

[crates.io: Rust Package Registry](https://crates.io/)

[MicroK8s - Zero-ops Kubernetes for developers, edge and IoT](https://microk8s.io/)

[Dynamic | Home - A powerful web3 auth developer platform.](https://www.dynamic.xyz/)

[Game Art Outsourcing Studio | Outstandly](https://www.outstandly.com/gamedev/)

[Local - Local WordPress development made simple](https://localwp.com/)

[Polybase — The web3 database](https://polybase.xyz/)

[Gradio](https://gradio.app/)

[GitHub - AminBhst/brisk: Fast, multi-threaded, cross-platform download manager](https://github.com/AminBhst/brisk)

[Typing](https://aymendn.github.io/typing-speed-test-flutter/#/)

[File uploading, processing & delivery for web and mobile apps | Uploadcare](https://uploadcare.com/)

[Where the Machine Creates the Code](https://aista.com/)

[godot-rust](https://godot-rust.github.io/)

[Zed - Code at the speed of thought](https://zed.dev/)

[React – The library for web and native user interfaces](https://react.dev/)

[.NET Boxed · GitHub](https://github.com/Dotnet-Boxed)

[Google Sheets API, Turn Google Spreadsheet Into API – Sheetsu](https://sheetsu.com/)

[Home - Lightrun](https://lightrun.com/)

[Starlane Studios · GitHub](https://github.com/StarlaneStudios)

[iBoxDB Pure Java .NET ACiD Embedded NoSQL Database](https://www.iboxdb.com/)

[Census Data Activation | Sync All Your Data Across All Your Tools](https://www.getcensus.com/)

[AI Colors](https://aicolors.co/)

[FastEndpoints](https://fast-endpoints.com/)

[GitHub - FastEndpoints/FastEndpoints: A light-weight REST API development framework for ASP.Net 6 and newer.](https://github.com/FastEndpoints/FastEndpoints)

[Float UI - Free open source Tailwind UI components & Templates](https://floatui.com/)

[IT Tools - Handy online tools for developers](https://it-tools.tech/)

[Vector Database for Vector Search | Pinecone](https://www.pinecone.io/)

[QuestPDF | QuestPDF](https://www.questpdf.com/)

[Home - zrok](https://zrok.io/)

[ngrok - Online in One Line](https://ngrok.com/)

[The edge cloud platform behind the best of the web | Fastly](https://www.fastly.com/)

[PikaPods - Instant Open Source App Hosting](https://www.pikapods.com/)

[mediastack - Free Live News & Blog Articles JSON API](https://mediastack.com/)

[marketstack - Real-Time & Historical Stock Data API](https://marketstack.com/)

[aviationstack - Real-Time Flight Tracker & Status API](https://aviationstack.com/)

[weatherstack - Real-Time World Weather REST API](https://weatherstack.com/)

[ipstack - Free IP Geolocation API](https://ipstack.com/)

[API Marketplace | Connect to Best APIs | APILayer](https://apilayer.com/)

[Typst: Compose papers faster](https://typst.app/)

[typst/typst: A new markup-based typesetting system that is powerful and easy to learn.](https://github.com/typst/typst)

[Fermyon • Experience the next wave of cloud computing. | Fermyon • Experience the next wave of cloud computing.](https://www.fermyon.com/)

[Bright Data - The World's #1 Web Data Platform](https://brightdata.com/)

[Pest | The elegant PHP testing framework](https://pestphp.com/)

[bloop | Find Code. Fast.](https://bloop.ai/)

[BloopAI/bloop: bloop is a fast code search engine written in Rust.](https://github.com/BloopAI/bloop)

[The fastest way to deploy applications globally - Koyeb](https://www.koyeb.com/)

[Postmark: Fast, Reliable Email Delivery Service | SMTP | API](https://postmarkapp.com/)

[Fast, Affordable Cloud Storage & Secure Data Protection - Wasabi](https://wasabi.com/)

[Nordic APIs](https://nordicapis.com/)

[Pushy - Lightning-Fast & Highly-Reliable Push Notifications](https://pushy.me/)

[Reach Creators and Developers at Scale | Carbon](https://www.carbonads.net/)

[Developer Ad Network with privacy built in - EthicalAds](https://www.ethicalads.io/)

[Tailscale · Best VPN Service for Secure Networks](https://tailscale.com/)

[Fab | A Digital Asset Marketplace for Content Creators](https://www.fab.com/)

[Ansible is Simple IT Automation](https://www.ansible.com/)

[Puppet Infrastructure & IT Automation at Scale | Puppet by Perforce](https://www.puppet.com/)

[Introducing the Crown | Neurosity](https://neurosity.co/)

[kysely-org/kysely: A type-safe typescript SQL query builder](https://github.com/kysely-org/kysely)

[drizzle-team/drizzle-orm: TypeScript ORM for SQL](https://github.com/drizzle-team/drizzle-orm)

[Depot](https://depot.dev/)

[Keyri – The authentication and fraud identity suite](https://keyri.com/)

[Authsignal - Home](https://www.authsignal.com/)

[All your data, all the time - Axiom](https://axiom.co/)

[Twilio Segment - Ranked #1 Customer Data Platform for 2021 market share, IDC 2022](https://segment.com/)

[The API to Video | Mux](https://www.mux.com/)

[sawhney17/logseq-schrodinger: A plugin to export pages in Logseq to Hugo.](https://github.com/sawhney17/logseq-schrodinger)

[ProseMirror](https://prosemirror.net/)

[Cursor | Build Fast](https://www.cursor.so/)

[SVG and PNG tech icons | TechIcons](https://techicons.dev/)

[Supercharged GitHub Experience · Volta](https://volta.net/)

[Adrenaline](https://useadrenaline.com/)

[binsta](https://binsta.dev/)

[Architect.io Home - Architect.io](https://www.architect.io/)

[Shields.io: Quality metadata badges for open source projects](https://shields.io/)

[Simple Icons](https://simpleicons.org/)

[A Collaborative List Of 700+ Resources For Developers](https://devresourc.es/)

[Skeleton — UI Toolkit for Svelte + Tailwind](https://www.skeleton.dev/)

[OKLCH Color Picker & Converter](https://oklch.com/#70,0.1,208,100)

[refine.new - Open-source enterprise application platform for serious web developers](https://refine.new/)

[Lago - Open Source Metering & Usage-Based Billing](https://www.getlago.com/)

[Tabby - a terminal for a more modern age](https://tabby.sh/)

[Create mock APIs in seconds with Mockoon](https://mockoon.com/)

[Coherence — The First Developer Experience Platform](https://www.withcoherence.com/)

[extrawurst/gitui: Blazing 💥 fast terminal-ui for git written in rust 🦀](https://github.com/extrawurst/gitui)

[jesseduffield/lazygit: simple terminal UI for git commands](https://github.com/jesseduffield/lazygit)

[Welcome · sidebase](https://sidebase.io/)

[Chroma](https://www.trychroma.com/)

[Arroyo - Serverless stream processing](https://www.arroyo.dev/)

[Login | PDFGen](https://pdfgen.app/login?redirectTo=/)

[Generate Images and PDFs with a Simple API and Highly Reusable Templates​ - APITemplate.io](https://apitemplate.io/)

[Talos Linux](https://www.talos.dev/)

[Video SDK for Real-time Communication, Live Streaming & Video API](https://www.videosdk.live/)

[Incident management for every developer | FireHydrant](https://firehydrant.com/)

[Hocus | Automated Self-Hosted Dev Environments](https://hocus.dev/)

[Home - Livebook.dev](https://livebook.dev/)

[Logflare | Cloudflare, Vercel & Elixir Logging](https://logflare.app/)

[Collaborative Infrastructure For Modern Software Teams](https://spacelift.io/)

[PartyKit: an open source platform for collaborative applications](https://partykit.io/)

[Val Town](https://www.val.town/)

[Fluree | Data Reimagined | Fluree](https://flur.ee/)

[dbpunk-labs/db3: DB3: open-source decentralized firebase firestore alternative](https://github.com/dbpunk-labs/db3)

[ZenStack - Prisma Catalyst For Full-stack Development | ZenStack](https://zenstack.dev/)

[Brendonovich/prisma-client-rust: Type-safe database access for Rust](https://github.com/Brendonovich/prisma-client-rust)

[Overview – rspc](https://www.rspc.dev/)

[The Graph](https://thegraph.com/en/)

[Step CI](https://stepci.com/)

[Dynamic Application & API Security Testing for Modern Teams](https://www.stackhawk.com/)

[GraphQL API for WordPress](https://graphql-api.com/)

[A developer productivity tooling platform. | moonrepo](https://moonrepo.dev/)

[TresJS | The solution for 3D on VueJS](https://tresjs.org/)

[Invertase - Developer tooling & Integration Specilists.](https://invertase.io/)

[InfraNodus: Generate Insight with AI and Network Thinking](https://infranodus.com/)

[Give your data team magical powers | Mage](https://www.mage.ai/)

[Discourse is the place to build civilized communities | Discourse - Civilized Discussion](https://www.discourse.org/)

[Gotify · a simple server for sending and receiving messages](https://gotify.net/)

[AeroGear - Open Source Libraries for Mobile Connectivity](https://aerogear.org/index.html)

[Strimzi - Apache Kafka on Kubernetes](https://strimzi.io/)

[Deta Space](https://deta.space/)

[Phase | Encrypt the signal. Store the noise.](https://phase.dev/)

[dbdev | The Database Package Manager](https://database.dev/)

[Confidential computing at scale for everyone | Edgeless Systems](https://www.edgeless.systems/)

[Flowbite Svelte - UI Component Library](https://flowbite-svelte.com/)

[Odin Programming Language](https://odin-lang.org/)

[Publisher of Ubuntu | Canonical](https://canonical.com/)

[The Data Cloud | Snowflake](https://www.snowflake.com/en/)

[Valorant-API](https://valorant-api.com/)

[Env](https://env.t3.gg/)

[The Virtual DevOps platform | mogenius](https://mogenius.com/)

[Contabo🥇Cloud VPS & Dedicated Servers for a Price You'll Love](https://contabo.com/en)

[Civo Kubernetes - Fast, Simple, Managed Kubernetes Service - Civo.com](https://www.civo.com/)

[Okteto - The Kubernetes development platform | Okteto](https://www.okteto.com/)

[Symbiosis – Managed Kubernetes](https://symbiosis.host/)

[Kraud Cloud – Free Managed Kubernetes](https://kraudcloud.com/)

[MichaelCade/90DaysOfDevOps: I am using this repository to document my journey learning about DevOps. I began this process on January 1, 2022, and plan to continue until March 31. I will be dedicating one hour each day, including weekends, to gaining a foundational understanding of the various aspects of DevOps. This will be a 90-day intensive study period. 2022 & 2023 inc.](https://github.com/MichaelCade/90DaysOfDevOps)

[Scarf I Home](https://about.scarf.sh/)

[Dev Community - DevDojo](https://devdojo.com/)

[BoxyHQ | Security Building Blocks for Developers](https://boxyhq.com/)

[Painless localization | Tolgee](https://tolgee.io/)

[Proton — Privacy by default](https://proton.me/)

[GitJournal](https://gitjournal.io/)

[Nestia Guide Documents - Index](https://nestia.io/docs/)

[fluentbit](https://fluentbit.io/)

[Parseable | Parseable](https://www.parseable.io/)

[uploadthing](https://uploadthing.com/)

[lemunozm/message-io: Fast and easy-to-use event-driven network library.](https://github.com/lemunozm/message-io)

[message-db/message-db: Microservice native message and event store for Postgres](https://github.com/message-db/message-db)

[Infisical | Open Source SecretOps](https://infisical.com/)

[Phind: AI search engine](https://www.phind.com/)

[Distributed Caching and Task Execution for Nx and Lerna](https://nx.app/)

[Unlighthouse - Site-wide Google Lighthouse · Unlighthouse](https://unlighthouse.dev/)

[Million.js](https://millionjs.org/)

[OrbStack · Fast, light, simple Docker & Linux on macOS](https://orbstack.dev/)

[WebContainers - Dev environments. In your web app. | WebContainers](https://webcontainers.io/)

[Brydge - the future of IoT development](https://www.brydge.dev/)

[Home Assistant](https://www.home-assistant.io/)

[Raindrop.io — All-in-one bookmark manager](https://raindrop.io/)

[Memex](https://memex.garden/)

[Omnivore](https://omnivore.app/)

[Future Tools - Find The Exact AI Tool For Your Needs](https://www.futuretools.io/)

[AI Music Generator - SOUNDRAW](https://soundraw.io/)

[Mubert - Thousands of Staff-Picked Royalty-Free Music Tracks for Streaming, Videos, Podcasts, Commercial Use and Online Content](https://mubert.com/)

[Best Free Text To Speech Voice Reader | Speechify](https://speechify.com/)

[ElevenLabs - Prime AI Text to Speech | Voice Cloning](https://beta.elevenlabs.io/)

[Jasper - AI Copywriter | AI Content Generator for Teams](https://www.jasper.ai/)

[dream-num/Luckysheet: Luckysheet is an online spreadsheet like excel that is powerful, simple to configure, and completely open source.](https://github.com/dream-num/Luckysheet)

[CodePen: Online Code Editor and Front End Web Developer Community](https://codepen.io/)

[Modular: AI development starts here](https://www.modular.com/)

[Apache Atlas – Data Governance and Metadata framework for Hadoop](https://atlas.apache.org/#/)

[Anvil | Build Web Apps with Nothing but Python](https://anvil.works/)

[Lucia](https://lucia-auth.com/)

[Poetry - Python dependency management and packaging made easy](https://python-poetry.org/)

[:: Anaconda.org](https://anaconda.org/)

[Refactoring and Design Patterns](https://refactoring.guru/)

[CourseLit - Sell online courses and digital downloads from your own website.](https://courselit.app/)

[Online Teaching Platform & LMS | Pupilfirst](https://www.pupilfirst.com/)

[The Discord Enhancement Project - BetterDiscord](https://betterdiscord.app/)

[Replicate – Run open-source machine learning models with a cloud API](https://replicate.com/)

[Realtime Colors](https://realtimecolors.com/?colors=000000-ffffff-4685ff-f2f2f2-ffb084)

[Lumin](https://www.uselumin.co/)

[Discord | Your Place to Talk and Hang Out](https://discord.com/)

[Discord Bots | The #1 Discord Bot List](https://top.gg/)

[ProBot - Bot for Discord](https://probot.io/)

[Studybot](https://www.studybot.ca/)

[Lighthouse](https://lighthouse-php.com/)

[Evervault — Eliminate the risk of data breaches](https://evervault.com/)

[LMS Experts | Award Winning LMS | Open LMS | Open LMS](https://www.openlms.net/)

[Open Source ERP and CRM | Odoo](https://www.odoo.com/)

[Masteriyo LMS WordPress Plugin: Create & Sell Online Courses](https://masteriyo.com/)

[WordPress LMS Plugin For Courses - Sensei LMS](https://senseilms.com/)

[Home - Open edX](https://openedx.org/)

[Instructure Learning Platform | Educational Software Development](https://www.instructure.com/)

[React Email](https://react.email/)

[Dub - Link Management for Modern Marketing Teams](https://dub.sh/)

[monday.com | A new way of working](https://monday.com/)

[The platform to build next‒gen apps | Airtable](https://www.airtable.com/)

[Manage your team’s work, projects, & tasks online • Asana](https://asana.com/)

[ByteByteGo | Technical Interview Prep](https://bytebytego.com/)

[Learn to code for free - Personal AI powered tutor 100+ interactive courses](https://codedamn.com/)

[AI Voice Generator: Versatile Text to Speech Software | Murf AI](https://murf.ai/)

[PDF Drive - Search and download PDF files for free.](https://www.pdfdrive.com/)

[mailgun/groupcache: Clone of golang/groupcache with TTL and Item Removal support](https://github.com/mailgun/groupcache)

[Strawberry - Zero-Dependency, Build-Free JavaScript Framework](https://strawberry.quest/)

[Quastor](https://www.quastor.org/)

[WeAreDevelopers - Developer Jobs & Event Platform](https://www.wearedevelopers.com/)

[draw.io](https://www.drawio.com/)

[Introduction – Presencejs](https://presence.js.org/)

[Paraphrasing Tool - QuillBot AI](https://quillbot.com/)

[BlockNote - Javascript Block-Based text editor | BlockNote](https://www.blocknotejs.org/)

[free plagiarism checker ::: plagium](https://www.plagium.com/en/plagiarismchecker)

[Onshape | Product Development Platform](https://www.onshape.com/en/)

[draw.io](https://app.diagrams.net/)

[DrizzleORM - next gen TypeScript ORM](https://orm.drizzle.team/)

[CAP](https://cap.dotnetcore.xyz/)

[Home | DTM tutorial](https://en.dtm.pub/)

[Data Lakehouse Architecture and AI Company - Databricks](https://www.databricks.com/)

[Table Convert Online - Make it easier to work with tables](https://tableconvert.com/)

[highlight.io: The open source monitoring platform.](https://www.highlight.io/)

[Ompluscator/dynamic-struct: Golang package for editing struct's fields during runtime and mapping structs to other structs.](https://github.com/Ompluscator/dynamic-struct)

[OPAL - ACCESS CONTROL | OPAL](https://www.opal.ac/)

[Tunnel - Dev tool for real-time product feedback](https://tunnel.dev/)

[AltStore](https://altstore.io/)

[Scoop](https://scoop.sh/)

[Home - Toon Boom Animation](https://www.toonboom.com/)

[NeetCode.io](https://neetcode.io/)

[IP Geolocation API & Free Address Database | DB-IP](https://db-ip.com/)

[Hyperbeam — Embed Virtual Computers in your website](https://hyperbeam.com/)

[Elementary Data - Open Source Data Observability](https://www.elementary-data.com/)

[Dimension](https://dimension.dev/)

[OpenReplay: Open-Source Session Replay & Analytics](https://openreplay.com/)

[Royalty Free Music by Beatoven | Beatoven.ai](https://www.beatoven.ai/)

[Productivity and Collaboration Software Suite | Zenkit](https://zenkit.com/)

[Inquery - Superpowers for SQL Databases](https://www.inquery.io/)

[Reflect Notes](https://reflect.app/)

[TypedWebhook.tools: a tool to test webhooks and generate payload types](https://typedwebhook.tools/)

[OpenObserve | Open Source Observability Platform for Logs, Metrics, Traces, and More – Your Ultimate Dashboard for Alerts and Insights](https://openobserve.ai/)

[Synadia](https://www.synadia.com/)

[zincsearch](https://zincsearch-docs.zinc.dev/)

[MetaCall Hub | Deploy code and Generate APIs for your services](https://metacall.io/)

[Sync your team's local .env variables - Envault](https://envault.dev/)

[Webhooks Gateway for sending and receiving webhooks - Convoy](https://getconvoy.io/)

[Resend](https://resend.com/)

[SST](https://sst.dev/)

[Datomic - Overview](https://www.datomic.com/)

[shadcn/ui](https://ui.shadcn.com/)

[SQLite Cloud - Share, scale and deploy SQLite databases.](https://sqlitecloud.io/)

[Turso | SQLite Developer Experience in an Edge Database](https://turso.tech/)

[Method Draw Vector Editor](https://editor.method.ac/)

[Filebox - File transfer plugin for websites](https://www.filebox.fm/)

[Filess.io | 100% Free Database Hosting forever](https://filess.io/)

[Scalingo - PaaS Cloud Hosting for developers - Scalingo](https://scalingo.com/)

[Website Builder - Create a Free Website Today | Wix.com](https://www.wix.com/)

[Knowledge management and documentation for code | Swimm](https://swimm.io/)

[Cloud Computing Services | VMs, Kubernetes, Storage | Akamai](https://www.linode.com/)

[DevPod - Open Source Dev-Environments-As-Code](https://devpod.sh/)

[Zapier | Automation that moves you forward](https://zapier.com/)

[PM2 - Home](https://pm2.keymetrics.io/)

[Advanced Load Balancer, Web Server, & Reverse Proxy - NGINX](https://www.nginx.com/)

[Devtron- The Kubernetes-Native Application Management Platform](https://devtron.ai/)

[Beeper — All your chats in one app. Yes, really.](https://www.beeper.com/)

[Elestio: Fully Managed Open source](https://elest.io/)

[Automate Your Business - Open Source Free Zapier Alternative - Self-hosted or Cloud - Activepieces](https://www.activepieces.com/)

[Scalable Feeds & Chat - Powerful APIs and Components by Stream](https://getstream.io/)

[Sequin - Sync APIs with Postgres](https://sequin.io/)

[Zuplo API Management](https://zuplo.com/)

[Basedash | The admin interface for your data](https://www.basedash.com/)

[React-Admin - The Open-Source Framework For B2B Apps](https://marmelab.com/react-admin/)

[Zapp! An online Dart & Flutter sandbox in the browser](https://zapp.run/)

[Deepnote: Analytics and data science notebook for teams.](https://deepnote.com/)

[Orama - Search, everywhere.](https://oramasearch.com/)

[Tremor – The React library to build dashboards fast](https://www.tremor.so/)

[nanostores/nanostores: A tiny (313 bytes) state manager for React/RN/Preact/Vue/Svelte with many atomic tree-shakable stores](https://github.com/nanostores/nanostores)

[UnoCSS: The instant on-demand Atomic CSS engine](https://unocss.dev/)

[Keats/jsonwebtoken: JWT lib in rust](https://github.com/Keats/jsonwebtoken)

[Slides – Create and Share Presentations for Free](https://slides.com/)

[Wonder Unit](https://wonderunit.com/)

[Zoho | Cloud Software Suite for Businesses](https://www.zoho.com/)

[Welcome to Undb.](https://www.undb.xyz/)

[Bun: SQL client for Golang](https://bun.uptrace.dev/)

[Skiff - Private, encrypted, secure email - 10 GB free](https://skiff.com/)

[Superhuman | Blazingly fast email for teams and individuals](https://superhuman.com/)

[Typia Guide Documents - Index](https://typia.io/docs/)

[Flightcontrol — AWS Without Pain](https://www.flightcontrol.dev/)

[Welcome!](https://dylibso.com/)

[Extism - make all software programmable. Extend from within. | Extism - make all software programmable. Extend from within.](https://extism.org/)

[Kirby is the CMS that adapts to you](https://getkirby.com/)

[Kratos](https://go-kratos.dev/en/)

[Diagram · Design tools from the future.](https://diagram.com/)

[Snapcraft - Snaps are universal Linux packages](https://snapcraft.io/)

[Home | Flathub](https://flathub.org/)

[Runblade | Secure Public Networking for Docker Containers](https://runblade.com/)

[dwmkerr/hacker-laws: 💻📖 Laws, Theories, Principles and Patterns that developers will find useful. #hackerlaws](https://github.com/dwmkerr/hacker-laws)

[Create Presentation Slides With AI in seconds with SlidesAI](https://www.slidesai.io/)

[Codeium · Free AI Code Completion & Chat](https://codeium.com/)

[jetpack-io/typeid: Type-safe, K-sortable, globally unique identifier inspired by Stripe IDs](https://github.com/jetpack-io/typeid)

[Jetpack.io | Tools for sophisticated cloud development](https://www.jetpack.io/)

[\[OFFICIAL\] EdrawMax Online: All-in-One Diagram Maker & Software](https://www.edrawmax.com/)

[Adroit-Abhik/Adroit-Scheduler: TIme table generator](https://github.com/Adroit-Abhik/Adroit-Scheduler)

[Writesonic - Best AI Writer, Copywriting & Paraph…](https://writesonic.com/)

[Neon - Electrify your Node with the power of Rust! | Neon](https://neon-bindings.com/)

[Metlo](https://www.metlo.com/)

[Vaadin | Discover the Web App Framework Built for Java](https://vaadin.com/)

[Docker Hub](https://hub.docker.com/)

[KairosDB](https://kairosdb.github.io/)

[Apache Iceberg](https://iceberg.apache.org/)

[Drift](https://drift.simonbinder.eu/)

[Antidote DB](https://www.antidotedb.eu/)

[Home | Lando](https://lando.dev/)

[Hybridly - Server-driven, client-rendered applications](https://hybridly.dev/)

[colab.google](https://colab.google/)

[Rapid Enterprise Mobile Apps: Build, Connect, Manage with Gluon](https://gluonhq.com/)

[My Drive - Google Drive](https://drive.google.com/drive/my-drive)

[Google Slides](https://docs.google.com/presentation/u/0/)

[Google Docs](https://docs.google.com/document/u/0/)

[Google Sheets](https://docs.google.com/spreadsheets/u/0/)

[Google Forms](https://docs.google.com/forms/u/0/)

[Kuma UI](https://www.kuma-ui.com/)

[Hibernate. Everything data.](https://hibernate.org/)

[Gradle - Plugins](https://plugins.gradle.org/)

[Maven Central](https://central.sonatype.com/)

[Charm](https://charm.sh/)

[Educative: Interactive Courses for Software Developers](https://www.educative.io/)

[Wing Programming Language for the Cloud](https://www.winglang.io/)

[Discover, Explore, and Find the Perfect NPM Packages](https://easynpm.netlify.app/)

[Unleash: Open-Source Feature Management for Enterprises](https://www.getunleash.io/?utm_source=daily.dev&utm_medium=cpc&utm_campaign=Demand_052023&utm_content=CodeFreelyReleaseConfidently)

[Ladle | Ladle](https://ladle.dev/)

[Conquer the coding interview. Master algorithm and data structure. Tech interview prep. LeetCode made easy. Free practice programming interview questions.](https://algo.monster/)

[Free Design Tool: Presentations, Video, Social Media | Canva](https://www.canva.com/)

[Primitives – Radix UI](https://www.radix-ui.com/)

[Go Prisma – Prisma Client Go](https://goprisma.org/)

[Prisma Client Rust](https://prisma.brendonovich.dev/)

[Forefront Chat](https://chat.forefront.ai/)

[Resume Matcher | Free Open Source ATS Tool To Match Resumes To Job Descriptions](https://www.resumematcher.fyi/)

[A new era of authentication - Kinde](https://kinde.com/)

[WorkOS — Your app, Enterprise Ready.](https://workos.com/)

[Get started with Fx | Fx](https://uber-go.github.io/fx/get-started/)

[Rix - The AI-powered search engine optimized for developers](https://hashnode.com/rix?ref=producthunt)

[Iris Web Framework](https://www.iris-go.com/)

[Sentio](https://www.sentio.xyz/)

[zCloud - Cloud. Made easy.](https://zcloud.ws/)

[Iconbuddy — 100K+ open source icons](https://iconbuddy.app/)

[Rest.li - A framework for building RESTful architectures at scale](https://linkedin.github.io/rest.li/)

[Hook0 — Open-Source Webhooks-as-a-service (WaaS)](https://www.hook0.com/?pricing.destination=cloud)

[Home | Solace - PubSub+ for Developers](https://www.solace.dev/)

[Consul by HashiCorp](https://www.consul.io/)

[Vault by HashiCorp](https://www.vaultproject.io/)

[Nomad by HashiCorp](https://www.nomadproject.io/)

[Packer by HashiCorp](https://www.packer.io/)

[Waypoint by HashiCorp](https://www.waypointproject.io/)

[Boundary by HashiCorp](https://www.boundaryproject.io/)

[Game Developer](https://www.gamedeveloper.com/)

[Ts.ED - A Node.js and TypeScript Framework on top of Express/Koa.js.](https://tsed.io/)

[Fern | SDKs for your API](https://www.buildwithfern.com/)

[How to GraphQL - The Fullstack Tutorial for GraphQL](https://www.howtographql.com/)

[Dev toolkit for building collaborative editors – Tiptap](https://tiptap.dev/)

[Optimize & Secure Your Cloud](https://www.rapidfort.com/)

[Discover the best APIs and SaaS products | API Tracker](https://apitracker.io/)

[TinyBase](https://tinybase.org/)

[NextUI - Beautiful, fast and modern React UI Library](https://nextui.org/)

[Wes Bos - Full Stack Developer](https://wesbos.com/)

[Level Up Tutorials - Learn modern web development](https://levelup.video/)

[Learn In-Demand Skills. Get Hired. Advance Your Career. | Zero To Mastery](https://zerotomastery.io/)

[Frontend Masters — Learn JavaScript, React, Vue & Angular from Masters of Front-End Development! | Frontend Masters](https://frontendmasters.com/)

[Learn to Code with Interactive Tutorials - Scrimba.com](https://scrimba.com/)

[CodingCat.dev](https://codingcat.dev/)

[Become a Full Stack Dev Faster ⚡️ - Galaxies.dev](https://galaxies.dev/)

[XORM - Simple and Powerful ORM for Go](https://xorm.io/)

[K3s](https://k3s.io/)

[Unified Team Collaboration Tool - Spike](https://www.spikenow.com/)

[eM Client | The Best Email Client for Windows and Mac](https://www.emclient.com/)

[Thunderbird — Free Your Inbox. — Thunderbird](https://www.thunderbird.net/en-US/)

[Transloadit: File uploading & processing API](https://transloadit.com/)

[Data Grid: AG Grid: High-Performance React Grid, Angular Grid, JavaScript Grid](https://www.ag-grid.com/)

[MUI: The React component library you always wanted](https://mui.com/)

[Nokori - The Universal Backend Platform for Every Project](https://nokori.com/)

[{{ mustache }}](https://mustache.github.io/)

[Create beautiful images of your code](https://ray.so/)

[Uppy](https://uppy.io/)

[Open Source Cloud Computing Infrastructure - OpenStack](https://www.openstack.org/)

[Bleve -- Bleve](https://blevesearch.com/)

[Watermill](https://watermill.io/)

[Go kit - A toolkit for microservices](https://gokit.io/)

[Next-Generation Event Streaming Platform | Memphis.dev](https://memphis.dev/)

[Perplexity AI](https://www.perplexity.ai/)

[ChatPDF - Chat with any PDF!](https://www.chatpdf.com/)

[tawk.to – "100% FREE live chat software for your website!"](https://www.tawk.to/)

[Rambox | Workspace Simplifier to Boost your Productivity](https://rambox.app/)

[Station • One app to rule them all](https://getstation.com/)

[kamranahmedse/design-patterns-for-humans: An ultra-simplified explanation to design patterns](https://github.com/kamranahmedse/design-patterns-for-humans)

[The New Stack | DevOps, Open Source, and Cloud Native News](https://thenewstack.io/)

[EbookFoundation/free-programming-books: :books: Freely available programming books](https://github.com/EbookFoundation/free-programming-books)

[List of Free Learning Resources In Many Languages | free-programming-books](https://ebookfoundation.github.io/free-programming-books/)

[donnemartin/system-design-primer: Learn how to design large-scale systems. Prep for the system design interview. Includes Anki flashcards.](https://github.com/donnemartin/system-design-primer)

[Pinterest](https://www.pinterest.com/)

[Sym: Secure Your Production Infrastructure](https://symops.com/)

[Clojure](https://clojure.org/)

[The Elixir programming language](https://elixir-lang.org/)

[Learn to Code Online | Treehouse](https://teamtreehouse.com/)

[Meetup | Find Local Groups, Events, and Activities Near You](https://www.meetup.com/)

[Haskell Language](https://www.haskell.org/)

[Home | Adoptium](https://adoptium.net/)

[Svelte DevTools+](https://www.sveltedevtools.com/)

[Home | Vulkan | Cross platform 3D Graphics](https://www.vulkan.org/)

[LottieFiles: Download Free lightweight animations for website & apps.](https://lottiefiles.com/)

[Windmill | Open source platform to build internal tools with scripts](https://www.windmill.dev/)

[Cascadeur - the easiest way to animate AI-assisted keyframe animation software](https://cascadeur.com/)

[OfferZen – The developer job marketplace](https://www.offerzen.com/)

[Home | Vue Devtools](https://devtools.vuejs.org/)

[getonboard.dev](https://www.getonboard.dev/)

[the next-generation, cross‐platform package manager](https://tea.xyz/)

[SpacetimeDB](https://spacetimedb.com/)

[NucliaDB, the vector database for HuggingFace users](https://nucliadb.com/)

[Sourcegraph](https://sourcegraph.com/search)

[Wondershare Filmora - Easy-to-Use Video Editing Software](https://filmora.wondershare.com/)

[Android Emulator on the Cloud and cross-platform - Genymotion](https://www.genymotion.com/)

[Continuous Integration and Delivery (CI/CD) Platform | Bitrise](https://bitrise.io/)

[Square | Solutions For Your Small, Medium & Large Business](https://squareup.com/us/en?v=all)

[Project IDX](https://idx.dev/)

[Streamlit • A faster way to build and share data apps](https://streamlit.io/)

[Home | GraphQL AI Model for text inputs & AI Art prompts](https://graphqlai.com/)

[Graphene-Python](https://graphene-python.org/)

[Pterodactyl](https://pterodactyl.io/)

[Learn to Code for Free Online - App Academy Open](https://www.appacademy.io/course/app-academy-open)

[Elixir news and status updates from the community for Elixir and Phoenix](https://elixirstatus.com/)

[Digma Runtime Linter](https://digma.ai/)

[Shorebird](https://shorebird.dev/)

[Courses crafted for the real world - Dometrain](https://dometrain.com/)

[Firecamp - Get instant API playgrounds to test Rest, GraphQL, WebSocket and SocketIO APIs](https://firecamp.io/)

[GraphQL Voyager](https://graphql-kit.com/graphql-voyager/)

[Cachier](https://cachierql.com/)

[Snowplow: Create first-party customer data in real time.](https://snowplow.io/)

[Monorepo Explained](https://monorepo.tools/)

[Zag - Rapidly build UI components without sweating over the logic. - Zag](https://zagjs.com/)

[EVE Online | The #1 Free Space MMORPG | Play here now!](https://www.eveonline.com/)

[seaborn.heatmap — seaborn 0.12.2 documentation](https://seaborn.pydata.org/generated/seaborn.heatmap.html)

[Six Labors : Six Labors](https://sixlabors.com/)

[Cloud Monitoring as a Service | Datadog](https://www.datadoghq.com/)

[Insights Capture — recording software for gaming 💥](https://insights.gg/)

[nodb.sh](https://nodb.sh/)

[UBER ADDRESS](https://uberaddress.io/)

[Matplotlib — Visualization with Python](https://matplotlib.org/)

[Goo Engine — DillonGoo Studios](https://www.dillongoostudios.com/gooengine)

[IBM/fp-go: functional programming library for golang](https://github.com/IBM/fp-go)

[FileGator - Free, Multi-User PHP File Manager](https://filegator.io/)

[Descope | Authentication and user management platform](https://www.descope.com/)

[Skich | New way to discover mobile games](https://skich.app/)

[Stability AI](https://stability.ai/)

[Modal](https://modal.com/)

[Starlane Studios](https://starlane.studio/)

[The No Infrastructure Platform as a Service | Aptible](https://www.aptible.com/)

[The doc for everyone, replace dozens of apps - Coda](https://coda.io/)

[Resilio File Sync Software | Unify, Control, and Accelerate Global Enterprise File Workflows](https://www.resilio.com/)

[Home | Monero - secure, private, untraceable](https://www.getmonero.org/)

[pythonnet/pythonnet: Python for .NET is a package that gives Python programmers nearly seamless integration with the .NET Common Language Runtime (CLR) and provides a powerful application scripting tool for .NET developers.](https://github.com/pythonnet/pythonnet)

[Rspack](https://www.rspack.dev/)

[MJML - The Responsive Email Framework](https://mjml.io/)

[NuGet Gallery | Home](https://www.nuget.org/)

[Apache APISIX® -- Cloud-Native API Gateway](https://apisix.apache.org/)

[Real-time ETL | Estuary](https://estuary.dev/)

[Zap](https://www.zapzsh.org/)

[Laragon - portable, isolated, fast & powerful universal development environment for PHP, Node.js, Python.](https://laragon.org/)

[Fast, disk space efficient package manager | pnpm](https://pnpm.io/)

[Home page | Yarn](https://yarnpkg.com/)

[Bit. Composable software.](https://bit.dev/)

[devowl.io: Quality WordPress Plugins & Developer Tools](https://devowl.io/)

[ElevenLabs - Generative AI Text to Speech & Voice Cloning](https://elevenlabs.io/)

[Draftbit - Visually build native mobile apps](https://draftbit.com/)

[carbondesignsystem.com](https://carbondesignsystem.com/)

[The World’s Weather Intelligence Platform](https://www.tomorrow.io/)

[Email testing and debugging has never been easier – HELO](https://usehelo.com/)

[The code runner for PHP 💫 - Tinkerwell](https://tinkerwell.app/)

[shadcn-svelte](https://www.shadcn-svelte.com/)

[Superforms for SvelteKit](https://superforms.rocks/)

[Laravel App Builder](https://laravelappbuilder.com/)

[Valibot: The modular and type safe schema library](https://valibot.dev/)

[BioDrop - connect to your audience with a single link](https://www.biodrop.io/)

[TailPress - Rapidly build your WordPress theme with Tailwind CSS](https://tailpress.io/)

[A Simple and Safer Way to Pay and Get Paid | PayPal LK](https://www.paypal.com/lk/home)

[Homepage | Net Ninja](https://netninja.dev/)

[Home ⋆ Asterisk](https://www.asterisk.org/)

[caisy - content made easy](https://caisy.io/)

[Wasmer - Run, Publish & Deploy any code, anywhere](https://wasmer.io/)

[DiceBear | Open Source Avatar Library](https://www.dicebear.com/)

[Learn Ruby on Rails | GoRails](https://gorails.com/)

[Introduction · 💾 Unstorage](https://unstorage.unjs.io/)

[CodeEdit](https://www.codeedit.app/)

[aria2](https://aria2.github.io/)

[Mend.io (formerly WhiteSource) | Improving AppSec Outcomes](https://www.mend.io/)

[Judge0 - Where code happens.](https://judge0.com/)

[Outerbase | The interface for your database](https://outerbase.com/)

[OpenCopilot - AI Copilot for your SaaS product](https://opencopilot.so/)

[Rig.dev | The open-source application platform for Kubernetes](https://rig.dev/)

[Faust.js™](https://faustjs.org/)

[Stack Sorted.](https://stacksorted.com/buttons)

[Start and grow your e-commerce business - 3-Day Free Trial - Shopify USA](https://www.shopify.com/)

[Spline - 3D Design tool in the browser with real-time collaboration](https://spline.design/)

[Elysia - Fast, and friendly Bun web frameworks | ElysiaJS](https://elysiajs.com/)

[JangaFX - Real-Time VFX Software For Real-Time VFX Artists](https://jangafx.com/)

[KitBash3D - Premium 3D Assets for Video Games, Film, and Television](https://kitbash3d.com/)

[Tekton](https://tekton.dev/)

[Flux](https://fluxcd.io/)

[LanceDB - developer friendly, serverless vector database](https://lancedb.com/)

[Compile SQL to type-safe code | sqlc.dev](https://sqlc.dev/)

[volatiletech/sqlboiler: Generate a Go ORM tailored to your database schema.](https://github.com/volatiletech/sqlboiler)

[DevGPT - Start Developing The Easy Way with AI-Generated Code](https://www.devgpt.com/)

[Intuitive and affordable motion capture tools for character animation](https://www.rokoko.com/)

[appium/appium: Cross-platform automation framework for all kinds of your apps built on top of W3C WebDriver protocol](https://github.com/appium/appium)

[PlayCSS - Daily challenges to test and improve your skills with fun](https://playcss.app/)

[Cirrus CSS 💎](https://www.cirrus-ui.com/)

[Liquid template language](https://shopify.github.io/liquid/?shpxid=885a9c93-97D8-4595-204C-8BB697C88D09)

[Open Source Software Projects - Shopify](https://shopify.github.io/)

[Shopify Developers Platform—Build. Innovate. Get paid.](https://shopify.dev/)

[CasaOS - A simple, easy-to-use, elegant open-source personal cloud system](https://casaos.io/)

[Npgsql - .NET Access to PostgreSQL | Npgsql Documentation](https://www.npgsql.org/)

[OctoBase - Local-first, yet collaborative database](https://octobase.pro/)

[BlockSuite | Collaborative Editing Toolkit](https://blocksuite.affine.pro/)

[Novel – Notion-style WYSIWYG editor with AI-powered autocompletions](https://novel.sh/)

[Introduction - Slate](https://docs.slatejs.org/)

[About - Puro](https://puro.dev/)

[CodeCrafters | Advanced programming challenges](https://codecrafters.io/)

[The all-in-one platform as a service (PaaS) | Platform.sh](https://platform.sh/)

[Code Push - Over the air updates for Flutter Apps](https://codepush.dev/)

[Nue / Frontend troublesolver](https://nuejs.org/)

[Nebula](https://nebula.tv/)

[lotusdblabs/lotusdb: Most advanced key-value store written in Go, extremely fast, compatible with LSM tree and B+ tree, optimization of badger and bbolt.](https://github.com/lotusdblabs/lotusdb)

[Tabler Icons: over 4650 vector icons for web design](https://tabler-icons.io/)

[contrib.rocks](https://contrib.rocks/preview?repo=angular%2Fangular-ja)

[Kickstarter](https://www.kickstarter.com/)

[Uizard | App, Web, & UI Design Made Easy | Powered By AI](https://uizard.io/)

[Rust Adventure](https://www.rustadventure.dev/)

[Home - MEGA](https://mega.io/)

[Scala CLI | Scala CLI](https://scala-cli.virtuslab.org/)

[Home Page](https://www.crossplane.io/)

[ElectricSQL - Sync for modern apps](https://electric-sql.com/)

[Actual](https://actualbudget.com/)

[PowerSync: SQL data sync for offline-first apps](https://www.powersync.co/)

[DriftDB | DriftDB](https://driftdb.com/)

[OpenTofu](https://opentofu.org/)

[OAuth Tools](https://oauth.tools/)

[Hello from Hydro-SDK | Hydro-SDK](https://hydro-sdk.io/)

[Databases, SQL Server, and Data Models Examples](https://datamodels.databases.biz/)

[Tembo](https://tembo.io/)

[Rogue Engine](https://rogueengine.io/)

[Kamal — Deploy web apps anywhere](https://kamal-deploy.org/)

[ABP Framework - Open Source Web Application Framework](https://abp.io/)

[Free Online Form Builder | Tally](https://tally.so/)

[Fleet](https://fleet.rs/)

[Diversion - Cloud-Native Version Control](https://www.diversion.dev/)

[Prismane / Make great React apps, ultra fast](https://www.prismane.io/)

[Graphite - How the fastest developers ship code](https://graphite.dev/)

[Introduction - FastGQL](https://www.fastgql.io/)

[Mailtrap: Email Delivery Platform](https://mailtrap.io/)

[Transactional Email API Service For Developers | Mailgun](https://www.mailgun.com/)

[EmailEngine Email API](https://emailengine.app/)

[Ethereal Email](https://ethereal.email/)

[Home ⚡ Zig Programming Language](https://ziglang.org/)

[sonner.emilkowal.ski](https://sonner.emilkowal.ski/)

[react-hot-toast - The Best React Notifications in Town](https://react-hot-toast.com/)

[AWESOME-APP - Rust Blueprint for Building Awesome Applications (Tauri, SurrealDB, Axum, SQLx, Postgresql, Kubernetes)](https://awesomeapp.dev/)

[InversifyJS a powerful IoC container for JavaScript apps powered by TypeScript](https://inversify.io/)

[Ferrous Systems](https://ferrous-systems.com/)

[Overview - Crux: Cross-platform app development in Rust](https://redbadger.github.io/crux/)

[Rid | Rust Integrated Dart](https://thlorenz.com/rid-site/)

[Unkey](https://unkey.dev/)

[XState - JavaScript State Machines and Statecharts](https://xstate.js.org/)

[Redux - A predictable state container for JavaScript apps. | Redux](https://redux.js.org/)

[Next.Nav](https://www.next-nav.com/?ref=dailydev)

[upper/db · A productive data access layer for Go](https://upper.io/v4/)

[jmoiron/sqlx: general purpose extensions to golang's database/sql](https://github.com/jmoiron/sqlx)

[scylladb/gocqlx: All-In-One: CQL query builder, ORM and migration tool](https://github.com/scylladb/gocqlx)

[doug-martin/goqu: SQL builder and query library for golang](https://github.com/doug-martin/goqu)

[Masterminds/squirrel: Fluent SQL generation for golang](https://github.com/Masterminds/squirrel)

[FL0 - backend engineering, supercharged](https://www.fl0.com/)

[HTTPie – API testing client that flows with you](https://httpie.io/)

[bruno](https://www.usebruno.com/)

[Restfox](https://restfox.dev/)

[Contentlayer makes content easy for developers](https://contentlayer.dev/)

[muse.ai | The power of video made simple for you.](https://muse.ai/)

[GitHub - httptoolkit/react-reverse-portal: Build an element once, move it anywhere](https://github.com/httptoolkit/react-reverse-portal)

[Home - @use-gesture documentation](https://use-gesture.netlify.app/)

[Poimandres · GitHub](https://github.com/pmndrs)

[Dracula — Dark theme for 300+ apps](https://draculatheme.com/)

[FFmpeg.guide - One stop solution to all things FFmpeg](https://ffmpeg.guide/)

[Kysely | Kysely](https://kysely.dev/)

[Home | Twind](https://twind.dev/)

[Hono - Ultrafast web framework for the Edges](https://hono.dev/)

[Ree.js | Ree.js](https://ree.js.org/)

[restQL - microservice query language](http://restql.b2w.io/)

[About](https://docs.restql.b2w.io/#/)

[PDFgear - Bring Accessible PDF Software to the Masses](https://www.pdfgear.com/)

[A Go package for building Progressive Web Apps](https://go-app.dev/)

[2:10:59 PM](https://www.sqlchat.ai/)

[Overview - pressly/goose](https://pressly.github.io/goose/)

[Meaningful Code Tests for Busy Devs | CodiumAI](https://www.codium.ai/)

[SingleAPI - Convert any website to an API in seconds.](https://singleapi.co/#pricing)

[Kinsta® Cloud Hosting - All Your Projects in One Platform](https://kinsta.com/)

[Open-source authentication platform - Fief](https://www.fief.dev/)

[kapa.ai - ChatGPT for your developer-facing product](https://www.kapa.ai/)

[Continuous delivery & deployment automation tool for DevOps teams | Octopus Deploy - Octopus Deploy](https://octopus.com/)

[ClamAVNet](https://www.clamav.net/)

[Open Swoole: PHP Server with Async IO, Coroutines and Fibers (previously Swoole) | Open Swoole PHP](https://openswoole.com/)

[Webhooks As A Service - HostedHooks](https://www.hostedhooks.com/)

[OpenFunction](https://openfunction.dev/)

[A Python library for GraphQL | 🍓 Strawberry GraphQL](https://strawberry.rocks/)

[Home](https://omnigres.com/)

[Orchid ORM | Orchid ORM](https://orchid-orm.netlify.app/)

[open-runtimes/open-runtimes: Open runtime environments for serverless cloud computing ⚡️](https://github.com/open-runtimes/open-runtimes)

[Epsio | Instant query results, effortlessly](https://www.epsio.io/)

[Bob - Lightning fast build platform](https://bob.build/)

[WebSocket King client: A testing and debugging tool for WebSockets](https://websocketking.com/)

[Home - ID-2Sbo](https://ssdb.io/)

[Wolverine | Next Generation .NET Mediator and Message Bus](https://wolverine.netlify.app/)

[Mock Service Worker - API mocking library for browser and Node.js](https://mswjs.io/)

[surreal-search.vercel.app](https://surreal-search.vercel.app/)

[Surrealist Query](https://surrealist.app/query)

[Robusta](https://home.robusta.dev/)

[OpenTelemetry](https://opentelemetry.io/)

[The Node-Based UI for Svelte – Svelte Flow](https://svelteflow.dev/)

[JSON Web Tokens - jwt.io](https://jwt.io/)

[Gravatar - Globally Recognized Avatars](https://gravatar.com/)

[RoboHash](https://robohash.org/)

[Hyperaide](https://hyperaide.com/)

[Hookdoo Homepage](https://hookdoo.com/index.html)

[PeerDB](https://www.peerdb.io/)

[Nango - Open Unified API](https://www.nango.dev/)

[Angular](https://angular.dev/)

[packagecloud.io](https://packagecloud.io/)

[Tarantool middleware for data](https://www.tarantool.io/en/)

[Home - Playground AI](https://playground.ai/)

[Pixlr: Free Online Photo Editor, AI image Generator & Design tool](https://pixlr.com/)

[Leonardo.Ai](https://leonardo.ai/)

[Helping people become more creative.](https://ideogram.ai/)

[Adobe Firefly](https://firefly.adobe.com/)

[Rinf Docs](https://rinf-docs.cunarist.com/)

[Acorn](https://www.acorn.io/)

[Deuxfleurs: let’s build a user-friendly internet](https://deuxfleurs.fr/)

[Next-Generation Intelligent Assistant - Safyrus](https://www.safyrus.codes/)

[Atuin - Magical Shell History](https://atuin.sh/)

[Ollama](https://ollama.ai/)

[The Valley of Code](https://thevalleyofcode.com/)

[Float UI - Free open source Tailwind UI components & Templates](https://www.floatui.com/)

[iLoveIMG | The fastest free web app for easy image modification.](https://www.iloveimg.com/)

[Xano - The Scalable No-Code Backend](https://www.xano.com/)

[Robust Job Processing · Oban Pro](https://getoban.pro/)

[Programiz: Learn to Code for Free](https://www.programiz.com/)

[About CRDTs • Conflict-free Replicated Data Types](https://crdt.tech/)

[ob1.outerbase.com/?ref=producthunt](https://ob1.outerbase.com/)

[AuthKit by WorkOS](https://www.authkit.com/)

[Holochain | Distributed app framework with P2P networking](https://www.holochain.org/)

[API Dash - Test, Visualize & Integrate APIs in a Dash ⚡️](https://apidash.dev/)

[Advent of Code 2023](https://adventofcode.com/)

[TallStackUI](https://tallstackui.com/)

[Livepeer - Home](https://livepeer.org/)

[pgxman](https://pgxman.com/)

[NotebookLM](https://notebooklm.google.com/)

[Hathora | Server Orchestration for Multiplayer Games](https://hathora.dev/)

[GPUX](https://gpux.ai/)

[flethy | time to speed up](https://flethy.com/)

[CrabNebula – Home](https://crabnebula.dev/)

[Capacities – A studio for your mind](https://capacities.io/)

[NSQ Docs 1.2.1 - A realtime distributed messaging platform](https://nsq.io/)

[The AT Protocol](https://atproto.com/)

[Space and Time | Verifiable Compute Layer for Web3](https://www.spaceandtime.io/)

[Data DX: A manifesto for effortlessly building data-driven applications](https://www.datadx.io/)

[Fluvio - The programmable data streaming platform](https://fluvio.io/)

[TypeHero](https://typehero.dev/)

[Interview Pen | Software Engineering Interviews, Data Structures & Algorithms ++ System Design](https://interviewpen.com/)

[Mapbox | Maps, Navigation, Search, and Data](https://www.mapbox.com/)

[Loco | Loco](https://loco.rs/)

[Graph Analytics Platform | Graph Database | TigerGraph](https://www.tigergraph.com/)

[MemCachier |](https://www.memcachier.com/)

[HarperDB | Enterprise Application Platform](https://www.harperdb.io/)

[CrateDB – The Hyper-Fast Database that Truly Scales](https://cratedb.com/)

[Playground - free-to-use online AI image creator](https://playgroundai.com/)

[LABS.GOOGLE](https://labs.google/)

[Luma AI](https://lumalabs.ai/)

[AI Chat for scientific PDFs | SciSpace](https://typeset.io/)

[Upscayl - AI Image Upscaler](https://www.upscayl.org/)

[GitHub - localtunnel/localtunnel: expose yourself](https://github.com/localtunnel/localtunnel)

[Open source Supabase SaaS starter - Basejump](https://usebasejump.com/)

[Auth UI | Universal Auth Service](https://www.authui.site/)

[StyleX](https://stylexjs.com/)

[Iroh](https://iroh.computer/)

[Eraser – Docs and Diagrams for Engineering Teams](https://www.eraser.io/)

[TS Docs | Reference docs for npm packages](https://tsdocs.dev/)

[GPTRouter - The Ultimate Model Router for GPT and AI Models | Writesonic](https://gpt-router.writesonic.com/)

[GameDev.tv Courses - Learn to Code Games -Blender, Unreal, C#, C++](https://www.gamedev.tv/)

[Ubicloud - Open and portable cloud](https://www.ubicloud.com/)

[Serverpod – The Flutter Backend](https://serverpod.dev/)

[Waves.js](https://wavesjs.github.io/)

[Arc from The Browser Company](https://arc.net/)

[Caravaggio | Caravaggio](https://caravaggio.ramielcreations.com/)

[Bytescale: Image, Video & Audio APIs for Developers](https://www.bytescale.com/)

[Mighty Image - Get your images in all shapes and sizes on demand](https://mightyimage.io/)

[Essential products for developers | Voidr](https://voidr.co/)

[River - Fast, reliable background jobs in Go](https://riverqueue.com/)

[MassTransit · MassTransit](https://masstransit.io/)

[Upbound - The Universal Cloud-Native Platform](https://www.upbound.io/)

[Eden AI | AI made easy for all Product Builders](https://www.edenai.co/)

[Vald](https://vald.vdaas.org/)

[Suno AI](https://www.suno.ai/)

[Expr | Expression language](https://expr-lang.org/)

[Rhai – Embedded Scripting for Rust](https://rhai.rs/)

[Genql - SDK client for any GraphQL API](https://genql.dev/)

[pest. The Elegant Parser](https://pest.rs/)

[Tornado Web Server — Tornado 6.4 documentation](https://www.tornadoweb.org/en/stable/#)

[Scaledrone Realtime Messaging Service](https://www.scaledrone.com/)

[Wasp](https://wasp-lang.dev/)

[A lightweight carousel library with fluid motion and great swipe precision | Embla Carousel](https://www.embla-carousel.com/)

[Conduktor | Universal Tooling for Apache Kafka](https://www.conduktor.io/)

[News API – Search News and Blog Articles on the Web](https://newsapi.org/)

[NewsData - News API to Search & Collect Worldwide News](https://newsdata.io/)

[The Local AI Playground](https://www.localai.app/)

[Cron Atlas - modern cron job scheduler for serverless apps](https://cronatlas.com/)

[GNews API: Your gateway to the power of news APIs](https://gnews.io/)

[Submit JSON](https://www.submitjson.com/)

[dyrector.io @ platform](https://dyrectorio.com/)

[OpenStatus](https://www.openstatus.dev/)

[Mistral AI | Open-weight models](https://mistral.ai/)

[Anthropic \\ Home](https://www.anthropic.com/)

[Fast & Free AI & GPTs Bots store | FlowGPT](https://flowgpt.com/)

[Massed Compute](https://shop.massedcompute.com/)

[Graphweaver - Data Everywhere, Instant GraphQL](https://graphweaver.com/)

[Gravitee.io - API Management Platform](https://www.gravitee.io/)

[uploadstuff.dev](https://uploadstuff.dev/)

[Inngest - Build reliable products - Durable workflow engine](https://www.inngest.com/)